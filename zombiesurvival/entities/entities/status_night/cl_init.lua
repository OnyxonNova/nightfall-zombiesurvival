...
local function GetRandomBonePos(pl)
    if pl ~= LocalPlayer() or pl:ShouldDrawLocalPlayer() then
...
function ENT:Draw()
    ...
    local pos
    if ent == LocalPlayer() and not ent:ShouldDrawLocalPlayer() then
    ...
    for i = 1, 2 do
        ...
        particle:SetColor(150, 0, 255) -- установка фиолетового цвета
    end
