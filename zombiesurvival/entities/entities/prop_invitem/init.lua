...
function ENT:GiveToActivator(activator, caller)
	...
	local pickupbetter = GAMEMODE.ZSInventoryItemData[itype].QualityTier > (activator:GetInventoryItemTier(itype) or 0)

	if itypecat == INVCAT_TRINKETS and pickupbetter and activator:GetInventoryItemTier(itype) ~= nil then
		local activatortrinket = (activator:GetInventoryItemTier(itype) ) < 1 and GAMEMODE:GetBaseItemName(itype) or GAMEMODE:GetBaseItemName(itype) .. "_q" .. activator:GetInventoryItemTier(itype)
		self:SetInventoryItemType(activatortrinket)
		activator:TakeInventoryItem(activator:GetInventoryItemTierData(itype))
		...
		return
	elseif (activator:HasInventoryItem(itype) or activator:HasInventoryItemTier(itype) or activator:HasInventoryItemTier(string.sub(itype,0,#itype-3)) or activator:HasInventoryItem(string.sub(itype,0,#itype-3))) then
