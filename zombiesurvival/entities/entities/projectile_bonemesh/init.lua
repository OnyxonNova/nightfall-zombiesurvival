...
function ENT:Explode(hitpos, hitnormal, hitent)
	...
	-- Добавляем код для применения статусов к игрокам
	for _, pl in pairs(ents.FindInSphere(hitpos, 90)) do
		if pl:IsValidLivingHuman() then
			pl:GiveStatus("slow", 2)
			local gt = pl:GiveStatus("enfeeble", 2)
	...
	for i = 1, 4 do
		for _, pl in pairs(ents.FindInSphere(hitpos, 90)) do
			if pl:IsValidLivingZombie() and not pl:GetStatus("zombie_regen") then
				...
				local ehp = zombieclasstbl.Boss and pl:GetMaxHealth() * 0.7 or pl:GetMaxHealth() * 1.25
