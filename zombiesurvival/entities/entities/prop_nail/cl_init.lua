...
function ENT:DrawTranslucent()
	...
	local nrp = self:GetRepairs()
	local mrps = self:GetMaxRepairs()
	--local nails = self:GetNailsCount()
	--local maxnails = GAMEMODE.MaxNails
	...
	if drawinfo then
		...
		if displayowner == "" then
			...
			if deployer:IsValid() then
				...
				if deployer:Team() == TEAM_HUMAN and deployer:Alive() then
					...
					displayowner = "(МЁРТВ) "..displayowner
					redname = true
				end
			end
		end
		...
		cam.Start3D2D(nearest, ang, 0.1)
			...
			surface.DrawRect(x + 1, y + 1, nrp/5 + nrp/50 - 3, hei - 2)
			...
			surface.DrawRect(x + 1, y + 1, nhp/5 + nhp/50 - 2, hei - 2)
			...
			if displayowner then
				...
				--draw.SimpleText("(" .. nails .. "/" .. maxnails .. ")", "ZS3D2DUnstyleSmallest", x + 90, y -36.5, Color(255, 255, 255, 125), TEXT_ALIGN_CENTER)
				draw.SimpleText(math.floor(nhp) .. "/" .. math.floor(mnhp), "ZS3D2DUnstyleTiny", x + 25, y - 25, Color(45, 255, 5, 240), TEXT_ALIGN_CENTER)
				draw.SimpleText(math.floor(nrp) .. "/" .. math.floor(mrps), "ZS3D2DUnstyleTiny", x + 25, y - 40, Color(100, 170, 215, 240), TEXT_ALIGN_CENTER)
			end
		cam.End3D2D()
