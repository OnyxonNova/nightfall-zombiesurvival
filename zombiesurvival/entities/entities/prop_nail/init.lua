...
local function PhysCallback(ent, data)
	if data.PhysObject:IsMotionEnabled() then
		for _, nail in pairs(ent:GetNails()) do
			local nailowner = nail:GetOwner()
			if nailowner and nailowner:IsValid() and nailowner:IsPlayer() and nailowner:Team() == TEAM_HUMAN then
				nail:Remove()
				nailowner:GiveAmmo(1, "GaussEnergy")
			end
		end
	end
	for _, nail in pairs(ent:GetCallbacks("PhysicsCollide")) do
		ent:RemoveCallback("PhysicsCollide", _)
	end
end

function ENT:OnRemove()
	local BaseEntGet = self:GetBaseEntity()
	local attachent = self:GetAttachEntity()
	BaseEntGet:AddCallback("PhysicsCollide", PhysCallback)
	if attachent:IsValid() then attachent:AddCallback("PhysicsCollide", PhysCallback) end
	local attachent = nil
	local BaseEntGet = nil
