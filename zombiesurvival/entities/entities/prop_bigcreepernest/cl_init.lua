...
local matCircle = Material("cable/redlaser")
function ENT:Draw()
...
function ENT:DrawHealthRadius()
	local pos = self:GetPos()
	local baseRadius = 100
	local pulseMagnitude = 25
	local pulseSpeed = 1
	local pulsingRadius = baseRadius + math.sin(CurTime() * pulseSpeed) * pulseMagnitude
	render.SetMaterial(matCircle)
	local color = Color(255, 0, 0)
	for i = 0, 360, 10 do
		local rad = math.rad(i)
		local x = math.cos(rad) * pulsingRadius
		local y = math.sin(rad) * pulsingRadius
		local startPos = pos + Vector(x, y, 0)
		local endPos = pos + Vector(math.cos(math.rad(i + 10)) * pulsingRadius, math.sin(math.rad(i + 10)) * pulsingRadius, 0)
