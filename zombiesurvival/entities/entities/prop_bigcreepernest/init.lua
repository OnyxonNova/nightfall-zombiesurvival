...
function ENT:Think()
    ...
    local nearbyEntities = ents.FindInSphere(self:GetPos(), 200)
    for _, ent in pairs(nearbyEntities) do
        if ent:IsPlayer() and ent:Team() == TEAM_UNDEAD then
            ...
            local baseHealth = classTab.Health + (healthPerWave) + (healthPerHuman * numHumanPlayers)
            local healthMultiplier = 1.45
            ...
            if currentHealth >= 0.6 * maxHealth then
                if not self.HealthyPlayers[ent:SteamID()] then
                    ent:SetHealth(math.min(currentHealth * healthMultiplier, maxHealth))
                    self.HealthyPlayers[ent:SteamID()] = true
                end
            end
        end
    end
...
hook.Add("PlayerDeath", "RemoveFromHealthyPlayers", function(player)
    for _, ent in pairs(ents.FindByClass("prop_bigcreepernest")) do
        if ent.HealthyPlayers then
            ent.HealthyPlayers[player:SteamID()] = nil
        end
    end
end)

hook.Add("PlayerSpawn", "GiveHealthAfterRespawn", function(player)
    for _, ent in pairs(ents.FindByClass("prop_bigcreepernest")) do
        if ent.HealthyPlayers then
            ent.HealthyPlayers[player:SteamID()] = nil
        end
...
function ENT:OnTakeDamage(dmginfo)
	...
	if attacker:IsValid() and attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
        local attackerClass = attacker:GetZombieClassTable().Name
        if attackerClass ~= "Flesh Creeper" and attackerClass ~= "Big Flesh Creeper" then
            return
        end

        local owner = self:GetNestOwner()
        if owner and owner:IsValidZombie() and owner ~= attacker and not attacker:IsAdmin() and owner:GetZombieClassTable().Name == "Big Flesh Creeper" and owner:Alive() and owner:GetPos():DistToSqr(self:GetPos()) <= 589824 then --768^2
	...
	if self:GetNestHealth() <= 0 then
		...
		gamemode.Call("NestDestroyed", self, attacker, dmginfo, self:GetNestBuilt())
