...
function ENT:RenderInfo(pos, ang, owner)
	cam.Start3D2D(pos, ang, 0.075)
		...
		if caches then
			draw.SimpleText(caches .. " " ..translate.Get("uses_left_resupply_box"), "ZS3D2DFont2Small", 0, 0, caches > 0 and COLOR_GREEN or COLOR_DARKRED, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
		...
		if MySelf:Team() == TEAM_HUMAN then
			...
			local icon = killicon.Get(GAMEMODE.AmmoIcons[GAMEMODE.CachedResupplyAmmoType])
            local iconget = isstring(icon[1]) and Material(icon[1]) or icon[1]
            local color = icon[2]

			surface.SetMaterial(iconget)
            surface.SetDrawColor(color)
            surface.DrawTexturedRect(180, 70 - 28, 90, 90)
		end
	cam.End3D2D()
end
...
net.Receive("zs_stowagecaches", function(length)
	MySelf.StowageCaches = net.ReadUInt(6)

	if net.ReadBool() then
		local ki = {--[[killiconcolor = COLOR_LIMEGREEN, ]]killicon = "weapon_zs_resupplybox"}
		GAMEMODE:CenterNotify(ki, " ", COLOR_LIMEGREEN, translate.Format("resupply_box_cache_added", MySelf.StowageCaches or 0))
	end
end)
