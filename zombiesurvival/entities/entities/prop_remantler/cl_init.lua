...
function ENT:RenderInfo(pos, ang, owner)
	cam.Start3D2D(pos, ang, 0.055)
		...
		if owner == MySelf then
			draw.SimpleText(self:GetScraps() .. (self:GetScraps() == 1 and translate.Get("rem_scrapone") or translate.Get("rem_scrap")), "ZS3D2DFont2Big", 0, 290, COLOR_GRAY, TEXT_ALIGN_CENTER)
		end
	cam.End3D2D()
end
