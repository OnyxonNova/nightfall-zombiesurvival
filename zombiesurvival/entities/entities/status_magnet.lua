...
function ENT:Think()
	...
	for _, ent in pairs(ents.FindInSphere(pos, activeweapon.Radius * activeweapon.MagnetStrength)) do
		...
		if ent and ent:IsValid() and self.Classes[class] and WorldVisible(pos, ent:NearestPoint(pos)) and droppedrecent and ownsitem then
			...
			phys:ApplyForceCenter(phys:GetMass() * (activeweapon.Force * activeweapon.MagnetStrength) * dir)
	...
	self:NextThink(CurTime() + activeweapon.ForceDelay / activeweapon.MagnetStrength)
	return true
end