...
function ENT:Initialize()
	...
	local healthMultiplier = 20
	
	local maxHealth = baseHealth + (numPlayers * healthMultiplier)

	self:SetMaxObjectHealth(maxHealth)
...
function ENT:Think()
	...
	if owner:IsValid() then
		...
		if fCurTime >= self.NextHeal and self.DamageTaken ~= 210 then
