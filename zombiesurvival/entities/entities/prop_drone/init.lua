...
function ENT:FireTurret(src, dir)
	if self:GetNextFire() <= CurTime() then
		...
		if curammo > 0 then
			...
			self:SetNextFire(CurTime() + self.TurretDelay)
			self:SetAmmo(curammo - 1)

			owner:LagCompensation(true)
			self:FireBulletsLua(src, dir, 5, 1, self.TurretDamage, owner, nil, "AR2Tracer", self.BulletCallback, nil, nil, self.GunRange, nil, self)
...
function ENT:Think()
	...
	if self.LastShadeLaunch and self.LastShadeLaunch + 2 > CurTime() and ent and ent:IsWorld() and CurTime() >= self.PhysDamageImmunity then
