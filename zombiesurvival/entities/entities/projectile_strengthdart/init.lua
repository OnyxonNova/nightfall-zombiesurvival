...
function ENT:Hit(vHitPos, vHitNormal, eHitEntity, vOldVelocity)
	...
	if eHitEntity:IsValid() then
		...
		if eHitEntity:IsPlayer() and eHitEntity:Team() ~= TEAM_UNDEAD then
			local strstatus = eHitEntity:GiveStatus(alt and "medrifledefboost" or "strengthdartboost", (alt and 1.5 or 1) * (self.BuffDuration or 10))
			strstatus.Applier = owner

			local txt = alt and "Защищающий Пистолет" or "Усиливающий Пистолет"
