...
ENT.TickTime = 0.5
ENT.DamageRadius = 250 -- Радиус, да бля

function ENT:Initialize()
    self:DrawShadow(false)
    self:Fire("attack", "", self.TickTime)

    if self:GetRadius() == 0 then self:SetRadius(self.DamageRadius) end
end
...
function ENT:AcceptInput(name, activator, caller, arg)
	...
	for _, ent in pairs(ents.FindInSphere(vPos, self.DamageRadius)) do
