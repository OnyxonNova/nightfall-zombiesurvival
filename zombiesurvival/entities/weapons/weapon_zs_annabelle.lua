...
SWEP.PrintName = "'Аннабель' Винтовка"
SWEP.Description = "Эта винтовка заряжает патроны по отдельности, что приводит к невысокой точности."
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Аннабель' Птичий Выстрел Винтовка", "Стреляет 6 пулями. Немного хуже точность, но увеличивается общий урон", function(wept)
	wept.Primary.Damage = wept.Primary.Damage / 5
	wept.Primary.NumShots = 6
	wept.ConeMin = wept.ConeMin * 8
	wept.ConeMax = wept.ConeMax * 2
end)
branch.NewNames = {[0]="Птичий Выстрел"}
