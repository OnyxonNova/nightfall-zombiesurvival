...
SWEP.PrintName = "'Конфликт' Глок 3"
SWEP.Description = "Стреляет 3 пулями за раз. Не очень точный, но очень большой урон вблизи."
...
SWEP.IronSightsPos = Vector(-5.782, 0, 1.55)
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Коллайдер' Глок", "На 1 пулю меньше, но точнее. После убийства урон увеличивается на 7%", function(wept)
...
branch.NewNames = {[0]="Коллайдер"}
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Саван' SOCOM Mark 23", "Стреляет 1 пулей, скрывает вашу ауру, меньше общего урона, но более точен", function(wept)
	wept.Primary.NumShots = 1
	wept.Primary.Damage = wept.Primary.Damage * 2.75
...
branch.Colors = {[0]=Color(205, 205, 205), [1]=Color(170, 170, 170), [2]=Color(120, 120, 120), [3]=Color(70, 70, 70)}
branch.NewNames = {[0]="Саван", [1]="Маскировка", [2]="Скрытый", [3]="Тихий"}
