...
if CLIENT then
	...
	SWEP.VElements = {
		["base"] = { type = "Model", model = "models/combine_scanner.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(5, 4, 0), angle = Angle(-54.206, 58.294, -50.114), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
	}
...
SWEP.DamageDrone = 26
SWEP.TurretDelay = 0.18

SWEP.NoDeploySpeedChange = true
SWEP.AllowQualityWeapons = true

GAMEMODE:SetPrimaryWeaponModifier(SWEP, WEAPON_MODIFIER_DAMAGE_DRONE, 4.35)
GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_TURRET_DELAY, -0.03)
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if ent:IsValid() then
			...
			ent.TurretDamage = self.DamageDrone
			ent.TurretDelay = self.TurretDelay
