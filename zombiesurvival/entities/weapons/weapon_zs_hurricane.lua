...
SWEP.PrintName = "'Ураган' Импульсный SMG"
SWEP.Description = "Ураган имеет хорошую скорострельность, и его пули замедляют цель."
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Тайфун' Импульсный SMG", "Меньше урона, больше точность, урон увеличивается при намотке", function(wept)
	wept.Primary.Damage = wept.Primary.Damage * 0.82
	wept.Primary.Delay = wept.Primary.Delay * 2.05
	wept.ConeMax = wept.ConeMax * 0.5
	wept.ConeMin = wept.ConeMin * 0.7
 wept.Primary.ClipSize = wept.Primary.ClipSize * 1.75
