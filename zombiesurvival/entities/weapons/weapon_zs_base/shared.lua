...
function SWEP:GetPrimaryClipSize()
	local owner = self:GetOwner()
	local multi = self.Primary.ClipSize/self.RequiredClip >= 8 and owner:IsSkillActive(SKILL_EXTENDEDMAG) and 1.15 or 1
...
function SWEP:FinishReload()
	...
	if max1 > 0 then
		...
		self:SetClip1(self.Primary.KnockbackScale == ZE_KNOCKBACKSCALE and max1 or (current + needed))
		if SERVER then
...
function SWEP:GetCone()
	...
	local tiervalidlow = (self.Tier or 1) <= 3
	local tiervalidhigh = (self.Tier or 1) >= 4
	local spreadmul = (owner.AimSpreadMul or 1) - ((tiervalidlow and owner:IsSkillActive(SKILL_REFINERAIM)) and 0.25 or 0) + ((tiervalidhigh and owner:IsSkillActive(SKILL_REFINERAIM)) and 0.25 or 0)

	if owner.TrueWooism then
		return (basecone + conedelta * 0.5 ^ self.ConeRamp) * spreadmul * orphic
	end

	if not owner:OnGround() or self.ConeMax == basecone then return self.ConeMax * 1.75 end

	local multiplier = math.min(owner:GetVelocity():Length() / self.WalkSpeed, 1) * 0.5

	local ironsightmul = 0.3 * (owner.IronsightEffMul or 1)
	local ironsightdiff = 0.25 - ironsightmul
	multiplier = multiplier + ironsightdiff

	if not owner:Crouching() then multiplier = multiplier + 0.3 end
...
function SWEP:ShootBulletsPiercing(dmg, numbul, cone)
	local owner = self:GetOwner()
	self:SendWeaponAnimation()
	owner:DoAttackEvent()
