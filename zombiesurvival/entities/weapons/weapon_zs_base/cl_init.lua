...
function DrawCharge3D(self, x, y, wid, hei)
	local add = 1.3
	local x = x * add
	local y = y * 1.1
	local wid = wid * add
	local hei = hei * add
	local barheight = 20 * 1.5
	...
	if fullcharge then
		local sin = math.abs(math.sin(CurTime() * 10)) * 80
		color = Color(color.r + sin, color.g + sin, color.b + sin, color.a)
	end

	local hitnumber = "ZS3D2DFont2SlightlySmallWep"
	local chargetext = fullcharge and "ZS3D2DFont2SlightlySmallWep" or "ZS3D2DFont2SlightlySmallWep"
	
	surface.SetDrawColor(colChargeBG)
	surface.DrawRect(x, bary, wid, barheight)
	surface.SetDrawColor(color)
	surface.DrawRect(x + minidel, bary + minidel, math.max(0, intd * (1.8 * add) - mini), barheight - mini)
	surface.SetDrawColor(Color(color.r * 0.8, color.g * 0.8, color.b * 0.8))
	surface.DrawRect(x + wid - 4, bary - minidel, mini / 1.75, barheight + mini)

	draw.SimpleTextBlurry(fullcharge and self.FullChargeText or self.ChargeText, chargetext, x + wid * 0.5, y + hei * 0.2, color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
...
function SWEP:Draw3DHUD(vm, pos, ang)
	...
	cam.Start3D2D(pos, ang, self.HUD3DScale / 2)
		...
		if self.EnableChargingAbility then
			local hei = 58
			DrawCharge3D(self, self.HUD3DScale / 2 - wid * 0.5 - hei/4, y - hei * 1.3, wid, hei)
		end
	cam.End3D2D()
end
...
function SWEP:CalcViewModelView(vm, oldpos, oldang, pos, ang)
	...
	local OffsetWeaponPos = Vector(GAMEMODE.WeaponPosX, GAMEMODE.WeaponPosZ, GAMEMODE.WeaponPosY)
	if Mul > 0 then
		local Offset = self.IronSightsPos - OffsetWeaponPos
	...
	pos:Add(ang:Right() * GAMEMODE.WeaponPosX)
    pos:Add(ang:Forward() * GAMEMODE.WeaponPosZ)
    pos:Add(ang:Up() * GAMEMODE.WeaponPosY)
