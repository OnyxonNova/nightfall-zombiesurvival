SWEP.PrintName = "Ядовитый Хедкраб"
...
function SWEP:Think()
	...
	elseif self:IsLeaping() then
		...
		if owner:IsOnGround() or 1 < owner:WaterLevel() then
			...
			local trace = owner:CompensatedMeleeTrace(owner:BoundingRadius() + self.MeleeSize, 12, vStart, owner:GetForward())
