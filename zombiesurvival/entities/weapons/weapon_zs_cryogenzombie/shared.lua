...
SWEP.PrintName = "Криогенный Скелет"

SWEP.MeleeDamage = 45
SWEP.SlowDownScale = 0.15
			...
			for _, spr in pairs(Spread) do
				...
				if ent:IsValid() then
					...
					if phys:IsValid() then
						phys:SetVelocityInstantaneous(ang:Forward() * 455)
					end
				end
			end
...
function SWEP:PlayAttackSound()
	self:EmitSound("npc/zombie_poison/pz_warn"..math.random(2)..".wav", 70, math.random(80, 90), 0.6, CHAN_AUTO)
	self:EmitSound("npc/metropolice/pain"..math.random(4)..".wav", 74, math.Rand(80, 90), 0.66, CHAN_WEAPON + 20)
end

function SWEP:PlayIdleSound()
	self:GetOwner():EmitSound("npc/antlion/idle"..math.random(5)..".wav", 70, math.random(20, 45))
end

function SWEP:PlayAlertSound()
	self:GetOwner():EmitSound("npc/stalker/breathing3.wav", 70, math.random(20, 45))
end
