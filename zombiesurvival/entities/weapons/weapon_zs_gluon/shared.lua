SWEP.PrintName = "'Гелиос' Глюонная Пушка"
SWEP.Description = "Стреляет потоком глюонов в цель, нанося огромный урон. Тяжело носить и стрелять, и накапливает тепло со временем."
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Аид' Пушка Хиггса", "Импульсные патроны замедляют цель, но наносит меньше урона", function(wept)
	...
	wept.LegDamaged = true
end)
branch.Colors = {[0]=Color(205, 205, 205), [1]=Color(160, 160, 160), [2]=Color(105, 105, 105), [3]=Color(50, 50, 50)}
branch.NewNames = {[0]="Аид", [1]="Глубокий", [2]="Нулевой", [3]="Войд"}

local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Тартар' Мюонная Пушка", "Наносит больше урона с помощью тепла, но может перегреться и поранить вас", function(wept)
...
branch.Colors = {[0]=Color(170, 125, 195), [1]=Color(150, 110, 180), [2]=Color(130, 90, 160), [3]=Color(110, 60, 150)}
branch.NewNames = {[0]="Тартар"}
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
	...
	if attacker:GetActiveWeapon().LegDamaged then
		local ent = tr.Entity
