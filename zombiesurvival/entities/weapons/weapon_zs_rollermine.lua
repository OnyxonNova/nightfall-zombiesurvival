...
SWEP.PrintName = "Шаровая Мина"
SWEP.Description = "Развертываемое устройство с дистанционным управлением.\nКатится по земле, отскакивая от зомби и нанося урон.\nМожет прыгать с помощью клавиши ПРОБЕЛ."
...
if CLIENT then
	...
	SWEP.VElements = {
		["base"] = { type = "Model", model = "models/roller.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(5, 4, 0), angle = Angle(-54.206, 58.294, -50.114), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
	}
...
SWEP.HitCooldown = 1.15
SWEP.DamageDrone = 52

GAMEMODE:SetPrimaryWeaponModifier(SWEP, WEAPON_MODIFIER_DAMAGE_DRONE, 8.75)
GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_HIT_COOLDOWN, -0.2)
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if ent:IsValid() then
			...
			ent.AmmoType = self.Primary.Ammo

			ent.HitCooldown = self.HitCooldown
			ent.HitDamage = self.DamageDrone
