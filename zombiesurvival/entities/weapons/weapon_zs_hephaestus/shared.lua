SWEP.PrintName = "'Гефест' Тау-Пушка"
SWEP.Description = "Также известен как пистолет Гаусса. Запускает тау-снаряды с невероятно высокой скоростью."
...
SWEP.ChargeDelay = 0.11

SWEP.Tier = 5
SWEP.MaxStock = 2

GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_FIRE_DELAY, -0.01)
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Прометей' Тау-Пушка", "Отскакивает под низкими углами, заряжается быстрее, увеличенная задержка огня", function(wept)
...
function SWEP:CheckCharge()
	if self:GetCharging() then
		...
		if not owner:KeyDown(IN_ATTACK2) then
			...
			self:ShootBullets((self.Primary.Damage * 0.7) * self:GetGunCharge(), self.Primary.NumShots, self:GetCone())
			self.IdleAnimation = CurTime() + self:SequenceDuration()
			self.FireAnimSpeed = 1

			local getvel = -34 * self:GetGunCharge() * owner:GetAimVector()
