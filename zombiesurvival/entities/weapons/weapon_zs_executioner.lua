...
SWEP.PrintName = "'Палач' Топор"
SWEP.Description = "Стабильный Топор с приятной дальностью атаки."
...
SWEP.SwingTime = 0.8
SWEP.SwingRotation = Angle(20, -20, -40)
...
function SWEP:PlayRevupSound()
    self:EmitSound("weapons/iceaxe/iceaxe_swing1.wav", 75, math.random(25, 30), 1)
...
function SWEP:PostOnMeleeHit(hitent, hitflesh, tr)
	if hitent:IsValid() and hitent:IsPlayer() and hitent:Alive() and hitent:Health() <= hitent:GetMaxHealthEx() * 0.10 and gamemode.Call("PlayerShouldTakeDamage", hitent, self:GetOwner()) then
		if SERVER then
