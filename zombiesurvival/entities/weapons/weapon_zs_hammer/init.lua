...
function SWEP:Reload()
	...
	local tr = owner:CompensatedMeleeTrace(self.MeleeRange * (owner.NailRangeMul or 1), self.MeleeSize)
	...
	if nailowner and nailowner:IsValid() and nailowner:IsPlayer() and nailowner ~= owner and nailowner:Team() == TEAM_HUMAN then
		...
		if nailowner:NearestPoint(tr.HitPos):DistToSqr(tr.HitPos) <= 589824 and (nailowner:HasWeapon("weapon_zs_hammer") or nailowner:HasWeapon("weapon_zs_electrohammer")) then
...
function SWEP:OnMeleeHit(hitent, hitflesh, tr)
	...
	if hitent:GetClass() == "prop_door_rotating" and owner:IsSkillActive(SKILL_HAMMERDOOR) then
		hitent:TakeDamage(400, owner, hitent)
	end
...
function SWEP:SecondaryAttack()
	local owner = self:GetOwner()
	if self:GetPrimaryAmmoCount() <= 0 or CurTime() < self:GetNextPrimaryFire() or (self:GetOwner():GetBarricadeGhosting() and not owner:IsSkillActive(SKILL_PHASENAILS)) then return end

	local tr = owner:CompensatedMeleeTrace(64 * (owner.NailRangeMul or 1), self.MeleeSize, nil, nil, nil, true)
	local trent = tr.Entity
	...
	for _, nail in pairs(ents.FindByClass("prop_nail")) do
		if nail:GetParent() == trent and nail:GetActualPos():DistToSqr(tr.HitPos) <= 80 - (owner.CompactNailMul or 0) then
