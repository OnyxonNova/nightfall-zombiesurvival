...
SWEP.PrintName = "'Дуб' Пистолет"
SWEP.Description = "Тяжелый пистолет который стреляет патронами что разлетаются на шрапнель при попадании на поверхность."
SWEP.Slot = 1
SWEP.SlotPos = 0
...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Ганфайтер' Пистолет", "Стреляет быстро, обойма большего размера, но наносит меньше урона", function(wept)
	wept.Primary.Damage = wept.Primary.Damage * 0.7
	wept.Primary.Delay = wept.Primary.Delay * 0.68
	wept.Primary.ClipSize = 16
	wept.WallDivide = 8
end)
branch.NewNames = {[0]="Ганфайтер"}
