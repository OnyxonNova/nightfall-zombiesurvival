...
SWEP.PrintName = "'Выброс' Пистолет"
SWEP.Description = "Стреляет очередями и чрезвычайно быстро заканчиваются патроны."
SWEP.Slot = 1
SWEP.SlotPos = 0
...
if CLIENT then
	...
	SWEP.HUD3DPos = Vector(-1.05, -2.75, -1.5)
...
SWEP.Primary.Damage = 28.5
SWEP.Primary.NumShots = 1
SWEP.Primary.Delay = 0.395
...
SWEP.IronSightsPos = Vector(-5, 0, 0.5)

SWEP.WalkSpeed = SPEED_NORMAL

GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_MAX_SPREAD, -0.185, 1)
GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_MIN_SPREAD, -0.125, 1)
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Выброс' Авто. Пистолет", "Делает пистолет полностью автоматическим за счет повреждений", function(wept)
...
branch.NewNames = {[0]="Авто. Пистолет"}
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Космос' Бластер", "Превращает это оружие в Импульсный бластер", function(wept)
...
branch.NewNames = {[0]="Космос", [1]="Импульсный", [2]="Орбитальный", [3]="Небесный"}
