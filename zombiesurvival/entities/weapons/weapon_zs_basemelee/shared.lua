...
SWEP.WalkSpeed = SPEED_BASENORMAL
...
SWEP.EnableChargingAbility = false

SWEP.FullChargeText = "ПКМ: "
SWEP.ChargeText = "Удары"
...
function SWEP:StartSwinging()
	...
	if self.StartSwingAnimation then
        ...
        local viewer = owner:GetViewModel()
        local cycle = viewer:GetCycle()
        if cycle > 0.75 then
            return
        end
        local sequence = self:SequenceDuration() * (1 - cycle)
        self:ResetSequenceInfo()
        viewer:ResetSequenceInfo()
        viewer:SetCycle(cycle)
        self:SetCycle(cycle)
        viewer:SetPlaybackRate(sequence / self.SwingTime)
...
function SWEP:MeleeSwing()
	...
	local tr = owner:CompensatedMeleeTrace(self.MeleeRange * (owner.MeleeRangeMul or 1) * (self.HealStrength and owner.NailRangeMul or 1), self.MeleeSize)
...
function SWEP:PlayerHitUtil(owner, damage, hitent, dmginfo)
	if owner.MeleePowerAttackMul and owner.MeleePowerAttackMul > 1 and not self.HealStrength then
...
function SWEP:PostHitUtil(owner, hitent, dmginfo, tr, vel)
	...
	if hitent:IsPlayer() then
		local knockback = self.MeleeKnockBack * 1.4 * (owner.MeleeKnockbackMultiplier or 1)
		if knockback > 0 then
...
function SWEP:MeleeHitEntity(tr, hitent, damagemultiplier)
	...
	if hitent:IsPlayer() then
		...
		local headMin = hitent:LocalToWorld(hitent:OBBMins() + Vector(-15, -15, hitent:OBBMaxs().z - 15))
        local headMax = hitent:LocalToWorld(hitent:OBBMaxs() + Vector(15, 15, 0))
		
		local IsHeadShot = (tr.HitPos:WithinAABox(headMin, headMax) or tr.HitGroup == HITGROUP_HEAD)

		if IsHeadShot and self.HeadshotMulti then
            damage = damage * self.HeadshotMulti
            dmginfo:SetDamage(damage)
        end
