SWEP.PrintName = "'Юность' Мед. Луч"
SWEP.Description = "Привязывается к выжившим и полностью лечит их, разряжая мед. бакты вдоль луча."
...
function SWEP:CheckHealRay()
	...
	if ent:IsValidLivingHuman() and gamemode.Call("PlayerCanBeHealed", ent) and owner:KeyDown(IN_ATTACK) and
		...
		if CurTime() > self:GetDTFloat(10) then
			...
			self:TakeAmmo()
			self:SetDTFloat(10, CurTime() + 0.30)
