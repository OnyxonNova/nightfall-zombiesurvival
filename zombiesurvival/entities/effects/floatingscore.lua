	...
	cam_Start3D2D(self.Pos + math_sin(CurTime() + self.Seed) * 30 * delta * right, ang, (delta * 0.12 + 0.045) / 2)
		...
		if amount == flooramount then
			if flag == 0 then
				...
				draw_SimpleText(flag == FM_LOCALKILLOTHERASSIST and " (помог)" or flag == FM_LOCALASSISTOTHERKILL and " (помощь)" or "", "ZS3D2DFont2", 0, 0, col, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			end
		else
			...
			if flag == 0 then
				...
				righttext = tostring(amount - flooramount):sub(2)..(flag == FM_LOCALKILLOTHERASSIST and " (помог)" or flag == FM_LOCALASSISTOTHERKILL and " (помощь)" or "")
			end
