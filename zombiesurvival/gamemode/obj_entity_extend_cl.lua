...
function meta:HealBloodPlayer(pl, amount)
	local healed, rmv = 0, 0
...
GM.CachedBigNests = {}
timer.Create("CachedBigNests", 0.5, 0, function()
	if not GAMEMODE then return end
	GAMEMODE.CachedBigNests = {}

	local nests = {}
	table.Add(nests, ents.FindByClass("prop_bigcreepernest"))

	GAMEMODE.CachedBigNests = nests
end)
...
timer.Create("CachedZSSpawnEntities", 2, 0, function()
	...
	if GAMEMODE:GetWave() < 3 and !GAMEMODE:GetWaveActive() then
