...
local function AltSelItemUpd()
	...
	GAMEMODE.InventoryMenu.SelectedItemLabel:CenterHorizontal(0.5)
end
...
net.Receive("zs_dmg", function(length)
	...
	if not GAMEMODE.DrawShotBloodParticles then
        local effectdata = EffectData()
        effectdata:SetOrigin(pos)
		effectdata:SetColor(victim:IsZombie() and victim:GetZombieClassTable().BloodColor or 0)
        util.Effect("BloodImpact", effectdata)
    end

	TotalDamage = TotalDamage + damage
	TotalDamageFade = CurTime() + 3
end)
...
net.Receive("zs_wavestart", function(length)
    ...
    elseif wave == GAMEMODE:GetNumberOfWaves() then
        ...
        if wave == 3 then
            GAMEMODE:CenterNotify(COLOR_RED, translate.Get("sigil_shield_deactivated"))
        end
    end
...
net.Receive("zs_waveend", function(length)
	...
	if wave < GAMEMODE:GetNumberOfWaves() and wave > 0 then
		...
		if MySelf:IsValid() and P_Team(MySelf) == TEAM_HUMAN then
			if GAMEMODE.LockItemTiers and GAMEMODE:GetWave() < (6 - GAMEMODE.LockItemsTierPlus) and GAMEMODE.LockItemsTierPlus < 5 then
				GAMEMODE:CenterNotify(COLOR_GREEN, translate.Format("tier_x_items_unlocked", GAMEMODE:GetWave() + (1 + (GAMEMODE.LockItemsTierPlus or 0))))
			end
...
net.Receive("zs_classunlock", function(length)
	GAMEMODE:CenterNotify(COLOR_GREEN, translate.Format("x_unlocked", net.ReadInt(6)))
end)
...
net.Receive("zs_healbloodother", function(length)
	if net.ReadBool() then
		gamemode.Call("HealedBloodOtherPlayer", net.ReadEntity(), net.ReadFloat())
	else
		GAMEMODE:CenterNotify({killicon = "weapon_zs_medicalkitblood"}, " ", Color(235, 65, 65), translate.Format("healed_x_for_y_blood", net.ReadEntity():Name(), net.ReadFloat()))
	end
end)
...
net.Receive("zs_sigilcorrupted", function(length)
	local corrupted = net.ReadUInt(2)
...
net.Receive("zs_invitem", function(length)
	...
	local inviname = GAMEMODE:GetTrinketName(GAMEMODE.ZSInventoryItemData[invitemt])
	local category = GAMEMODE:GetInventoryItemType(invitemt)

	surface.PlaySound("items/ammo_pickup.wav")
	GAMEMODE:CenterNotify({killicon = category == INVCAT_TRINKETS and GAMEMODE.ZSInventoryItemData[invitemt].Icon or "weapon_zs_trinket"}, " ", COLOR_RORANGE, translate.Format("obtained_a_inv", inviname))
end)
...
net.Receive("zs_invgiven", function(length)
	...
	GAMEMODE:CenterNotify({killicon = category == INVCAT_TRINKETS and GAMEMODE.ZSInventoryItemData[invitemt].Icon or "weapon_zs_trinket"}, " ", COLOR_RORANGE, translate.Format("obtained_inv_item_from_z", inviname, ent:Name()))
end)
...
net.Receive("zs_healbloodby", function(length)
	local amount = net.ReadFloat()
	local ent = net.ReadEntity()

	if not ent:IsValidPlayer() then return end

	GAMEMODE:CenterNotify({killicon = "weapon_zs_medicalkitblood"}, " ", Color(235, 65, 65), translate.Format("healed_x_by_y_blood", ent:Name(), amount))
end)
...
net.Receive("zs_currentround", function(length)
	GAMEMODE.CurrentRound = net.ReadUInt(2)
end)

net.Receive("zs_updatealtselwep", function(length)
	if MySelf:Alive() and P_Team(MySelf) == TEAM_HUMAN and GAMEMODE.InventoryMenu ~= nil and not GAMEMODE.InventoryMenu.SelInv then
