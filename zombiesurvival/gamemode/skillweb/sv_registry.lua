GM:SetSkillModifierFunction(SKILLMOD_HEALTH, function(pl, amount)
	...
	local newhp = 100 + math.Clamp(amount, -99, 1000)

	pl:SetMaxHealth(newhp)
	pl:SetHealth(math.max(1, pl:Health() / current * newhp))
end)
...
GM:AddSkillModifier(SKILL_REINFORCEDBLADES, SKILLMOD_MANHACK_HEALTH_MUL, -0.10)
...
GM:AddSkillModifier(SKILL_AVIATOR, SKILLMOD_CONTROLLABLE_HEALTH_MUL, -0.15)
...
GM:AddSkillModifier(SKILL_FORAGER, SKILLMOD_RESUPPLY_DELAY_MUL, 0.1)

GM:AddSkillModifier(SKILL_NAILSAMMUNITION, SKILLMOD_RESUPPLY_DELAY_MUL, 0.15)

GM:AddSkillModifier(SKILL_FIELDAMP, SKILLMOD_FIELD_DELAY_MUL, -0.1)
GM:AddSkillModifier(SKILL_FIELDAMP, SKILLMOD_FIELD_RANGE_MUL, -0.3)
