...
local REMORT_SKILL = {Name = "Перерождение", Description = "Идите вперёд.\nВы потеряете все навыки, опыт, скилл поинты и уровни.\nНачните с уровня 1, но с 1 дополнительными СП."}

local TREE_SKILLS = {
	[TREE_HEALTHTREE] = {Name = "Здоровье", Description = "Улучшите свою жизненную силу"},
	[TREE_SPEEDTREE] = {Name = "Ловкость", Description = "Повысьте свою ловкость"},
	[TREE_GUNTREE] = {Name = "Дальний бой", Description = "Будьте точным и эффективным"},
	[TREE_MELEETREE] = {Name = "Ближний бой", Description = "Увеличьте свою физическую мощь"},
	[TREE_BUILDINGTREE] = {Name = "Баррикадирование", Description = "Укрепите свою баррикаду"},
	[TREE_TECHNICIANTREE] = {Name = "Техник", Description = "Улучшайте свою технику и открывайте новые чертежи"},
	[TREE_SUPPORTTREE] = {Name = "Медицина", Description = "Поддержите благополучие других"}, 
	[TREE_BLOODARMORTREE] = {Name = "Кровавая Броня", Description = "Укрепите своё сопротивление"}, 
}
...
function PANEL:Init()
	...
	bottomleft:SetSize(200 * screenscale, 130 * screenscale)
	
	local bottomleftup = vgui.Create("DEXRoundedPanel", self)
	bottomleftup:DockPadding(10, 10, 10, 10)
	bottomleftup:SetSize(275 * screenscale, 145 * screenscale)
	...
	local dropdown = vgui.Create("DEXComboBox", bottomleft)
	dropdown:Dock(TOP)
	...
	savebtn.DoClick = function(me)
        ...
        local frame = DEXDerma_StringRequest(translate.Get("dsr_saveskills"), translate.Get("dsr_saveskillsname"), translate.Get("worth_menu_name"),
        function(strTextOut)
            SaveSkillLoadout(strTextOut)
            UpdateDropDown(dropdown)
            self:DisplayMessage(translate.Format("dsr_savedskills", strTextOut), COLOR_GREEN)
        end,
	...
	loadbtn.DoClick = function(me)
		...
		self:DisplayMessage("Загрузка навыков '" .. nlname .."' загружено!", COLOR_GREEN)
	end
	...
	activateall:SetText("Включить ВСЕ")
	...
	activateall.DoClick = function(me)
		...
		if #MySelf:GetUnlockedSkills() == 0 then
			self:DisplayMessage("У вас нет навыков для активации!", COLOR_RED)
		else
			self:DisplayMessage("Все разблокированные навыки активированы.", COLOR_GREEN)
		end
	...
	deactivateall:SetText("Выключить ВСЕ")
	...
	deactivateall.DoClick = function(me)
		...
		if #MySelf:GetUnlockedSkills() == 0 then
			self:DisplayMessage("У вас нет навыков для деактивации!", COLOR_RED)
		else
			self:DisplayMessage("Все разблокированные навыки деактивированы.", COLOR_RED)
		end
	...
	reset.DoClick = function(me)
        DEXDerma_Query(
            translate.Get("dq_refund"),
            translate.Get("dq_warning"),
            translate.Get("dsr_ok"),
            function() net.Start("zs_skills_reset") net.SendToServer() end,
            translate.Get("dsr_cancel"),
            function()
            end
        )
	end
	...
	spremaining:SetChangeFunction(function()
		return translate.Get("sk_unused")..MySelf:GetZSSPRemaining()
	end, true)
	...
	contextmenu:SetSize(200 * screenscale, 128 * screenscale)
	...
	button:SetSize(200 * screenscale, 32 * screenscale)
	button:AlignTop()
	...
	button.DoClick = function(me)
		...
		if MySelf:IsSkillDesired(skillid) then
			...
			self:DisplayMessage(name.." деактивирован.")
		elseif MySelf:IsSkillUnlocked(skillid) then
			...
			self:DisplayMessage(name.." активирован.", COLOR_DARKGREEN)
		else
			...
			self:DisplayMessage(name.." разблокирован и активирован!", COLOR_GREEN)
		end
	...
	warningtext:SetText("Изменения применяются при спавне!")
...
function PANEL:UpdateQuickStats()
	...
	local owner = self
	for i=1, 4 do
		local prefix = i == 1 and "Здоровье" or i == 2 and "Скорость" or i == 3 and "Нач. поинты" or i == 4 and "Кровавая броня"
		local val = i == 1 and 100 or i == 2 and SPEED_NORMAL or i == 3 and GAMEMODE.StartingWorth or i == 4 and 25
...
function PANEL:GenerateParticles()
	...
	for i=1, 140 do
		...
		particle = {}
		particle[1] = Vector(dist, math.Rand(-610, 610), math.Rand(-610, 610))
...
local particlecolors = {
	[TREE_HEALTHTREE] = Color(152, 255, 152, 90),
	...
	[TREE_TECHNICIANTREE] = Color(110, 110, 170, 90),
	[TREE_SUPPORTTREE] = Color(130, 200, 135, 90),
	[TREE_BLOODARMORTREE] = Color(255, 100, 100, 90)
}

local nodecolors = {
	[TREE_HEALTHTREE] = {3, 1.5, 6},
	[TREE_SPEEDTREE] = {2, 2, 5},
	[TREE_GUNTREE] = {5, 2, 2},
	[TREE_MELEETREE] = {3, 5, 6.75},
	[TREE_BUILDINGTREE] = {2, 6, 3},
	[TREE_TECHNICIANTREE] = {5.5, 4, 3},
	[TREE_SUPPORTTREE] = {3, 1.5, 6},
	[TREE_BLOODARMORTREE] = {1.5, 7, 6}
}
...
function PANEL:Paint(w, h)
	...
	local campos = self.vCamPos
	campos.x = math.Approach(campos.x, self.DesiredZoom, dt * 16000)
	self:SetCamPos(campos)
	...
	if mc == MOUSE_LEFT then
		...
		if hoveredskill then
			...
			elseif hoveredskill == -1 then
				self:DisplayMessage("Вы должны быть 50 уровня, чтобы сделать перерождение!", COLOR_RED)
			...
			elseif MySelf:IsSkillDesired(hoveredskill) then
				if GAMEMODE.Skills[hoveredskill].AlwaysActive then
					self:DisplayMessage("Вы не можете деактивировать этот навык!", COLOR_RED)
					surface.PlaySound("buttons/button8.wav")
						
					return
				end
					
				contextmenu.Button:SetText("Деактивировать")
			elseif MySelf:IsSkillUnlocked(hoveredskill) then
				contextmenu.Button:SetText("Активировать")
			elseif MySelf:SkillCanUnlock(hoveredskill) then
				if MySelf:GetZSSPRemaining() >= 1 then
					contextmenu.Button:SetText("Открыть")
				else
					self:DisplayMessage("Вам нужен СП, чтобы разблокировать этот навык!", COLOR_RED)
					surface.PlaySound("buttons/button8.wav")
						
					return
				end
			else
				self:DisplayMessage("Вам необходимо открыть все перечисленные требования!", COLOR_RED)
				surface.PlaySound("buttons/button8.wav")
...
function GM:DrawXPBar(x, y, w, h, xpw, barwm, hm, level)
	...
	if level == GAMEMODE.MaxLevel then
		draw_SimpleText(translate.Get("sk_max")..append, "ZSXPBar", xpw / 2, h / 2 + y, COLOR_GREEN, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	else
		if progress > 0 then
		...
		draw_SimpleText(translate.Get("sk_level")..level..append, "ZSXPBar", x, h / 2 + y, COLOR_WHITE, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw_SimpleText(string.CommaSeparate(xp).." / "..string.CommaSeparate(GAMEMODE:XPForLevel(level + 1))..translate.Get("sk_xp"), "ZSXPBar", x + barw, h / 2 + y, COLOR_WHITE, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end
...
function PANEL:PerformLayout()
	...
	if GAMEMODE.GameStatePanel and GAMEMODE.GameStatePanel:IsValid() then
		self:MoveBelow(GAMEMODE.GameStatePanel, screenscale * 16)
	else
		self:AlignTop(400)
	end
	self:AlignLeft()
...
function PANEL:Paint(w, h)
	if not GAMEMODE.HideShadowInterface then
	...
	if sp > 0 then
		...
		draw_SimpleText(sp..translate.Get("sk_sp"), "ZSHUDFontSmallest", w - 2, h / 2, colFlash, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end
