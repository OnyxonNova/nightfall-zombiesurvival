...
function GM:_CreateMove(cmd)
	...
	if myteam == TEAM_HUMAN then
		if MySelf:Alive() then
			...
			if lockon then
				if self:ValidMenuLockOnTarget(MySelf, lockon) and MySelf:KeyDown(self.MenuKey) then
				...
				local frightened = (MySelf:GetStatus("frightened") or MySelf:GetStatus("nightmarescare"))
