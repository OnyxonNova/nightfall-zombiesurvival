...
function PANEL:RefreshContents()
	...
	for k, nest in ipairs(GAMEMODE.CachedNests) do
		...
		local item = EasyButton(self, "Гнездо (" .. ownname .. " - " .. occurs[nown] .. ")", 8, 4)
	...
	for k, baby in ipairs(GAMEMODE.CachedBabies) do
		...
		local item = EasyButton(self, "Кукла", 8, 4)
		item:SetFont("ZSHUDFontSmall")
		item:SizeToContents()
		item.DoClick = function()
			net.Start("zs_nestspec")
	...
	for k, bignest in ipairs(GAMEMODE.CachedBigNests) do
		if not bignest:IsValid() then continue end
		local nown = bignest:GetNestOwner()
		occurs[nown] = (occurs[nown] or 0) + 1
		local ownname = nown:IsValidZombie() and nown:ClippedName() or ""

		local item = EasyButton(self, "Большое гнездо (" .. ownname .. " - " .. occurs[nown] .. ")", 8, 4)
		item:SetFont("ZSHUDFontSmall")
		item:SizeToContents()
		item.DoClick = function()
			net.Start("zs_nestspec")
				net.WriteEntity(bignest)
			net.SendToServer()
		end
