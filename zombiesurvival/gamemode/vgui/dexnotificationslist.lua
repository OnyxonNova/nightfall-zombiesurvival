...
function PANEL:AddKillIcon(class, customcolor)
	local icondata = killicon.GetIcon(class)

	if icondata then
		self:AddImage(icondata[1], customcolor or icondata[2])
	else
		local fontdata = killicon.GetFont(class) or killicon.GetFont("default")
		if fontdata then
			self:AddLabel(fontdata[2], customcolor or fontdata[3], fontdata[1], true)
		end
	end
end
	...
	for k, v in ipairs(args) do
		...
		if vtype == "table" then
			...
			elseif v.killicon then
				self:AddKillIcon(v.killicon, v.killiconcolor)
				if v.headshot then
