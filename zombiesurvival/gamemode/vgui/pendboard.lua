...
function MakepEndBoard(winner)
	...
	if localwin then
		...
		heading = EasyLabel(frame, "Вы Выиграли!", "ZSHUDFontBig", COLOR_CYAN)
	else
		surface.PlaySound("ambient/levels/citadel/strange_talk"..math.random(3, 11)..".wav")
		heading = EasyLabel(frame, "Вы Проиграли!", "ZSHUDFontBig", COLOR_RED)
	end
	...
	local subheading
	if localwin then
		subheading = EasyLabel(frame, "Выжившие смогли удержать оборону.", "ZSHUDFontSmaller", COLOR_WHITE)
	else
		subheading = EasyLabel(frame, "Зомби оказались сильнее.", "ZSHUDFontSmaller", COLOR_LIMEGREEN)
	end
	subheading:SetPos(wid * 0.5 - subheading:GetWide() * 0.5, y)
	y = y + subheading:GetTall() + 2

	local svpan = EasyLabel(frame, "Достижения", "ZSHUDFontSmall", COLOR_WHITE)
