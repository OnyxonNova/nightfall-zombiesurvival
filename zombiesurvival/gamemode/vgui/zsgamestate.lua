...
function PANEL:PerformLayout()
	...
	self:SetSize(screenscale * 825, screenscale * 80)
end
...
function PANEL:Text1Paint()
	...
	if override and #override > 0 then
		...
		if GAMEMODE:IsEscapeSequence() then
			text = translate.Get(MySelf:IsValid() and MySelf:Team() == TEAM_ZOMBIES and "prop_obj_exit_z" or "prop_obj_exit_h")
		elseif wave <= 0 then
			text = translate.Get("prepare_yourself")
				text = text .. " - " .. translate.Format("round_x", round)
		elseif GAMEMODE.ZombieEscape then
			...
			if maxwaves ~= -1 then
				text = translate.Format("wave_x_of_y", wave, maxwaves) .. " - " .. translate.Format("round_x", round)
				if not GAMEMODE:GetWaveActive() then
					text = translate.Format("intermission_wave_x_of_y", wave, maxwaves) .. " - " .. translate.Format("round_x", round)
				end
...
function PANEL:Text3Paint()
	...
	if MySelf:IsValid() then
		if MySelf:Team() == TEAM_UNDEAD then
			local zombietext = translate.Format("brains_eaten_x", MySelf:Frags())
			draw.SimpleText(zombietext, self.Font, 0, 0, COLOR_SOFTRED)

			local wid, hei = surface.GetTextSize(zombietext)
			local col = Color(200, 200, 200)
			col.a = 90 + math.abs(math.sin(RealTime() * 4)) * 160
...
function PANEL:Paint(w, h)
	if GAMEMODE.HideShadowInterface then return false end
	local wid = 420 * BetterScreenScale()
	surface.SetDrawColor(0, 0, 0, 180)
	surface.DrawRect(0, 0, wid * 0.4, h)
	surface.SetMaterial(matGradientLeft)
	surface.DrawTexturedRect(wid * 0.4, 0, wid * 0.6, h)

	surface.SetDrawColor(0, 0, 0, 250)
	surface.SetMaterial(matGradientLeft)
	surface.DrawTexturedRect(0, h - 1, wid, 1)
