...
function MakepCredits()
	...
	local label = EasyLabel(frame, "NightFall - " .. GAMEMODE.Name .. " " .. translate.Get("credits"), "ZSHUDFontNS", color_white)
	label:AlignTop(y)
	label:CenterHorizontal()
	y = y + label:GetTall() + 8

	for authorindex, authortab in ipairs(GAMEMODE.Credits) do
		if not authortab[5] then
			local avatar = vgui.Create("AvatarImage", frame)
			avatar:SetPos(4, y + 2)
			avatar:SetSize(44, 44)
			if authortab[4] ~= nil then
				avatar:SetSteamID(authortab[4], 44)
			end
			...
			if linesub then
				linesub:AlignTop(y)
				linesub:AlignLeft(58)
				y = y + linesub:GetTall()
			end
			y = y + 10
		else
			local lineleft = EasyLabel(frame, string.Replace(authortab[1], "@", "(at)"), "ZSHUDFontSmallestNS", color_white)
			local linemid = EasyLabel(frame, "Zombie Survival " .. translate.Get("credits"), "ZSHUDFontSmallNS", color_white)
			local lineright = EasyLabel(frame, authortab[3], "ZSHUDFontSmallestNS", color_white)
			local linesub
			if authortab[2] then
				linesub = EasyLabel(frame, authortab[2], "DefaultFont", color_white)
...
function MakepHelp()
	...
	local label = EasyLabel(Window, "Помощь", "ZSHUDFont", color_white)
	label:CenterHorizontal()
	label:AlignTop(0)

	local propertysheet = vgui.Create("DEXPropertySheet", Window)
	propertysheet:StretchToParent(12, 52, 12, 40)
	...
	for _, helptab in ipairs(GAMEMODE.Help) do
		...
		<body>
<center><span style="font-size:22px;font-weight:bold;color:limegreen;text-decoration:underline;">Зомби Выживание</span><br>
