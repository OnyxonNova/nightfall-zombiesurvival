...
AddCSLuaFile("cl_weaponselector.lua")
...
AddCSLuaFile("vgui/voicenotify.lua")

AddCSLuaFile("vgui/dexcheckbox.lua")
AddCSLuaFile("vgui/dexcombobox.lua")
AddCSLuaFile("vgui/dexmenu.lua")
AddCSLuaFile("vgui/dextooltip.lua")
AddCSLuaFile("vgui/dexpropertysheet.lua")
...
AddCSLuaFile("vgui/pdatabase.lua")
...
AddCSLuaFile("vgui/ppropmaker.lua")
AddCSLuaFile("vgui/pitemviewer.lua")
AddCSLuaFile("vgui/pitemdisplay.lua")
...
function GM:TryHumanPickup(pl, entity)
	if self.ZombieEscape or not pl:Alive() or pl:Team() ~= TEAM_HUMAN or (entity.NoPickupsTime and CurTime() < entity.NoPickupsTime and entity.NoPickupsOwner ~= pl) then return end
...
function GM:AddResources()
	...
	resource.AddFile("resource/fonts/remingtonnoiseless_upd.ttf")
...
function GM:AddNetworkStrings()
	...
	util.AddNetworkString("zs_healbloodother")
	util.AddNetworkString("zs_healbloodby")
	...
	util.AddNetworkString("zs_itemupdate")
	util.AddNetworkString("zs_invitemupdate")
	...
	util.AddNetworkString("zs_human_kill_nest")
	...
	util.AddNetworkString("zs_propmaker_send")
end
...
function GM:ShowTeam(pl)
	if pl:Team() == TEAM_HUMAN and not self.ZombieEscape then
		pl:SendLua((self.CheckedOut[pl:UniqueID()]) and "GAMEMODE:OpenArsenalMenu()" or "MakepWorth()")
	end
end
...
function GM:ShowSpare2(pl)
	pl:SendLua("GAMEMODE:MakeOptions()")
end
...
weaponmodelstoweapon["models/props_c17/metalpot001a.mdl"] = "weapon_zs_fryingpan"
...
weaponmodelstoweapon["models/props_junk/glassbottle01a.mdl"] = "weapon_zs_f_beer"
weaponmodelstoweapon["models/props_lab/ladel.mdl"] = "weapon_zs_fryingpan"
...
weaponmodelstoweapon["models/weapons/w_pickaxe/pickaxe01.mdl"] = "weapon_zs_pickaxe"
...
function GM:SetupProps()
	for _, ent in pairs(ents.FindByClass("prop_physics*")) do
		...
		if mdl then
			...
			if weaponmodelstoweapon[mdl] then
		...
		elseif self.DynamicSpawning then
			if self:ShouldUseAlternateDynamicSpawn() then
				for _, h in pairs(team.GetPlayers(TEAM_HUMAN)) do
					if h:GetPos():DistToSqr(epicenter or pl:GetPos()) < 1048576 then
				...
				if dyn then
					...
					if self:DynamicSpawnIsValid(dyn) then
						if dyn:GetClass() == "prop_creepernest" or dyn:GetClass() == "prop_bigcreepernest" then
...
function GM:SpawnBossZombie(bossplayer, silent, bossindex, triggerboss)
    ...
    if not bossplayer then
        bosses = self:CalculateNextBoss()
        if not bosses or #bosses == 0 then
            return
        end
        bossplayer = bosses[1] -- Берем только первого босса из списка
    else
        bosses = {bossplayer}
    end
    ...
    bossplayer.BossHealRemaining = 1200
...
function GM:Think()
    ...
    if not self.RoundEnded then
        ...
        elseif self:GetWaveStart() ~= -1 then
            ...
            elseif self.BossZombies and not self.ZombieEscape
            and self.LastBossZombieSpawned ~= wave and wave > 1 and not self.RoundEnded
            and (self.BossZombiePlayersRequired <= 0 or #player.GetHumans() >= self.BossZombiePlayersRequired) then
                local bosses = self:CalculateNextBoss()
                if bosses and #bosses > 0 and not self.AntiStallActive then
                    if self:GetWaveStart() - 5 <= time then
                        self:SpawnBossZombie(bosses[1]) -- Вызываем первого босса
                        if #player.GetHumans() >= self.SecondBossZombiePlayersRequired and wave > 3 then
                            timer.Simple(0, function() self:SpawnBossZombie(bosses[2]) end) -- Вызываем второго босса 
                        end
                    else
	...
	if NextTick <= time then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if pl:IsSkillActive(SKILL_REGENERATOR) and time >= pl.NextRegenerate and pl:Health() < math.min(pl:GetMaxHealth(), pl:GetMaxHealth() * 0.6) then
					pl.NextRegenerate = time + 6
					pl:SetHealth(math.min(pl:GetMaxHealth(), pl:Health() + 1))
				end

				if (pl:HasTrinket("regenimplant") or pl:HasTrinket("regenimplant_q1") or pl:HasTrinket("regenimplant_q2") or pl:HasTrinket("regenimplant_q3")) and time >= pl.NextRegenTrinket and pl:Health() < pl:GetMaxHealth() then
					pl.NextRegenTrinket = time + pl.RegenImplantTimer
					pl:SetHealth(math.min(pl:GetMaxHealth(), pl:Health() + 1))
				end

				pl:CheckTrinketRecharges()

				if pl:IsSkillActive(SKILL_CIRCULATION) and pl.circulationused and (not pl.LastCirculation or pl.LastCirculation + 60 < CurTime()) then
					if self:GetWave() > 0 then
						pl:SendLua("surface.PlaySound(\"buttons/button5.wav\")")
						pl:CenterNotify({killicon = "weapon_zs_trinket"}, " ", COLOR_RORANGE, translate.ClientFormat(pl, "skill_recharged", "Циркуляция"))
					end

					pl.circulationused = false
				end

				if pl:IsSkillActive(SKILL_LASTSTAND) and pl.laststandused and (not pl.LastLastStand or pl.LastLastStand + 140 < CurTime()) then
					if self:GetWave() > 0 then
						pl:SendLua("surface.PlaySound(\"buttons/button5.wav\")")
						pl:CenterNotify({killicon = "weapon_zs_trinket"}, " ", COLOR_RORANGE, translate.ClientFormat(pl, "skill_recharged", "Последний бой"))
					end

					pl.laststandused = false
				end

				if pl:HasTrinket("cartridgebelt_q3") and pl.OldWeaponToReload and time > (pl.NextAutomatedReload or 0) then
					local mywep = pl.OldWeaponToReload
					if mywep and mywep:IsValid() and mywep.FinishReload and not mywep.PrimaryHealingSound then
						...
						if max1 > 0 then
							...
							if needed > 0 then
								pl:CenterNotify({killicon = mywep:GetClass()}, " ", COLOR_CYAN, translate.ClientFormat(pl, "weapon_auto_reloaded", mywep.PrintName))
							end
						end
					end
				...
				if time > (pl.NextResupplyUse or 0) and self:GetWave() > 0 then
					...
					net.Start("zs_stowagecaches")
						net.WriteUInt(pl.StowageCaches, 6)
						net.WriteBool(true)
					net.Send(pl)
				end
			end
		...
		if self:GetEscapeStage() == ESCAPESTAGE_DEATH then
			for _, pl in pairs(allplayers) do
				if P_Team(pl) == TEAM_HUMAN then
					pl:TakeSpecialDamage(20, DMG_ACID)
				end
			end
		end
	end
...
function GM:PlayerSwitchWeapon(pl, old, new)
	if pl:HasTrinket("cartridgebelt_q3") then
...
function GM:CalculateNextBoss()
	...
	local newbosses = {}
	local minPlayersForSecondBoss = 17 
	if #zombies >= minPlayersForSecondBoss then
        for i = 1, 2 do
            if zombies[i] then
                table.insert(newbosses, zombies[i])
            end
        end
    else
        table.insert(newbosses, zombies[1])
    end
    if #newbosses > 0 then
        net.Start("zs_nextboss")
        net.WriteEntity(newbosses[1])
        net.WriteUInt(newbosses[1]:GetBossZombieIndex(), 8)
        if newbosses[2] then
            net.WriteEntity(newbosses[2])
            net.WriteUInt(newbosses[2]:GetBossZombieIndex(), 8)
        else
...
function GM:CalculateInfliction(victim, attacker)
	...
	local BannedModes = self.ZombieEscape or self.ObjectiveMap or self.RoundEnded
    
    if BannedModes then
        if self.AntiStallTimer or self.AntiStallUnlocking or self.AntiStallActive then
            self.AntiStallTimer = nil
            self.AntiStallUnlocking = false
            self.AntiStallActive = false
        end
    else
        local corruptedSigils = self:NumSigilsCorrupted() or 0
        local maxSigils = self.GetMaxSigils() or 1
        local currentWave = self:GetWave() or 0
        local humanPlayers = #player.GetHumans()
        local EscapingTime = self:GetUseSigils() and self:GetEscapeSequence() and self:GetEscapeStage() >= ESCAPESTAGE_BOSS
        
        if not self.AntiStallActive then
            if corruptedSigils >= maxSigils and currentWave > 3 and humanPlayers > 4 or EscapingTime then
                if not self.AntiStallUnlocking then
                    self.AntiStallUnlocking = true
                    self.AntiStallTimer = CurTime() + (EscapingTime and 0 or math.random(60, 75))
                end
            elseif corruptedSigils < maxSigils then
                if self.AntiStallUnlocking then
                    self.AntiStallUnlocking = false
                    self.AntiStallTimer = nil
                end
            end
        ...
        if self.AntiStallUnlocking and self.AntiStallTimer and CurTime() >= self.AntiStallTimer then
            self.AntiStallActive = true
            self.AntiStallUnlocking = false
            self.AntiStallTimer = nil
            
            for _, pl in pairs(player.GetAll()) do
                pl:CenterNotify({killicon = "Default"}, COLOR_RED, " ", {font = "ZSHUDFont"}, translate.ClientGet(pl, "super_zombies_unlocked"), {killicon = "Default"})
                pl:SendLua("surface.PlaySound(\"zombiesurvival/the_horror1.ogg\")")
            end
        end
    end
...
function GM:PlayerHealedBloodTeamMember(pl, other, health, wep, pointmul, nobymsg, floater)
	health = health - other:RemoveUselessDamage(health)
	if health <= 0 or pl == other then return end
	...
	if pointmul ~= 0 then
		if self:GetWave() != 0 then
			...
			pl:AddPoints(points * 0.75)
		end
	end

	net.Start("zs_healbloodother")
		net.WriteBool(not floater)
		net.WriteEntity(other)
		net.WriteFloat(health)
	net.Send(pl)
...
function GM:RestartRound()
	...
	net.Start("zs_currentround")
		net.WriteUInt(self.CurrentRound, 2)
	net.Broadcast()
...
function GM:RestartLua()
	...
	self.AntiStallUnlocking, self.AntiStallTimer, self.AntiStallActive = nil
...
function GM:ScalePlayerDamage(pl, hitgroup, dmginfo)
	...
	local lesser = attacker:IsSkillActive(SKILL_LEGFOCUS) and 0.75 or 0
	if not pl:CallZombieFunction2("ScalePlayerDamage", hitgroup, dmginfo) then
		if hitgroup == HITGROUP_HEAD then
			local multiplierhead = math.max(1, ((inflictor.HeadshotMulti or 2) - lesser))
			dmginfo:SetDamage(dmginfo:GetDamage() * multiplierhead * (attacker:IsPlayer() and attacker:GetStatus("renegade") and 1.1 or 1))
		elseif (hitgroup == HITGROUP_LEFTLEG or hitgroup == HITGROUP_RIGHTLEG) and not attacker:IsSkillActive(SKILL_LEGFOCUS) then
...
function GM:PlayerInitialSpawn(pl)
	pl:SetMaxBloodArmor(GAMEMODE.ZombieEscape and 0 or 10)
...
function GM:PlayerInitialSpawnRound(pl)
	...
	if pl:Team() == TEAM_UNDEAD and not self:GetWaveActive() then
		...
		pl:SetZombieClassName(setclass)
...
function GM:PropBroken(ent, attacker)
	if IsValid(ent) and IsValid(attacker) and not ent._PROPBROKEN and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN then
		...
		PrintMessage(HUD_PRINTCONSOLE, attacker:Nick() .. " ( " .. attacker:SteamID() .. " ) за выжившего сломал проп ( " .. ent:GetModel() .. " )")

	end
end

function GM:NestDestroyed(ent, attacker, dmginfo, builded)
	if IsValid(ent) and IsValid(attacker) and attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
		PrintMessage(HUD_PRINTCONSOLE, attacker:Nick() .. " ( " .. attacker:SteamID() .. " ) за муравья уничтожил гнездо на позиции: " .. tostring(ent:GetPos()) .. "")
	end
	
	if IsValid(ent) and IsValid(attacker) and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN and builded then
		net.Start("zs_human_kill_nest")
...
function GM:EntityTakeDamage(ent, dmginfo)
	...
	-- Props about to be broken props take 4x damage from anything except zombies
...
function GM:DamageFloater(attacker, victim, dmgpos, dmg, definiteply)
	...
	net.Start((definiteply or victim:IsPlayer()) and "zs_dmg" or "zs_dmg_prop")
		if definiteply or victim:IsPlayer() then
...
function GM:PlayerHurt(victim, attacker, healthremaining, damage)
	...
	if victim:Team() == TEAM_HUMAN then
		...
		if self:GetWave() > 0 then
			...
			if victim:IsSkillActive(SKILL_CIRCULATION) and not victim.circulationused and (not victim.LastCirculation or victim.LastCirculation + 60 < CurTime()) 
			and healthremaining < victim:GetMaxHealth() * 0.5 and victim:GetBloodArmor() <= victim:GetMaxBloodArmor() * 0.5 then

				victim:SetBloodArmor(math.min(victim:GetMaxBloodArmor(), victim:GetBloodArmor() + 20))

				victim.circulationused = true
				victim:EmitSound("buttons/button3.wav", 125, 30)
				victim:CenterNotify({killicon = "weapon_zs_trinket"}, " ", COLOR_GREEN, translate.ClientFormat(victim, "skill_triggered", "Циркуляция"))

				victim.LastCirculation = CurTime()
			end

			if victim:IsSkillActive(SKILL_LASTSTAND) and (not victim.LastLastStand or victim.LastLastStand + 140 < CurTime()) and healthremaining < victim:GetMaxHealth() * 0.35 then
				victim:GiveStatus("laststand", 60)

				victim.laststandused = true
				victim:EmitSound("items/smallmedkit1.wav", 125, 55)
				victim:CenterNotify({killicon = "weapon_zs_trinket"}, " ", COLOR_GREEN, translate.ClientFormat(victim, "skill_triggered", "Последний бой"))
				victim.LastLastStand = CurTime()
			end
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	if inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
		...
		if attacker.MeleeMovementSpeedOnKill and attacker.MeleeMovementSpeedOnKill ~= 0 and wep.IsMelee then
			local time = attacker:HasTrinket("adrenalinerush") and 3 or attacker:HasTrinket("adrenalinerush_q1") and 2 or attacker:HasTrinket("adrenalinerush_q2") and 1 or 0
			local boost = attacker:GiveStatus("adrenalineamp", 10 - time)
...
function GM:ZombieKilledHuman(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	attacker:AddZSXP(math.floor(xp/2))
	...
	if not pl.Gibbed and not suicide then
		...
		pl:SetZombieClassName(self.AntiStallActive and "Super Zombie ZS" or self.ZombieEscape and "Super Zombie ZE" or "Fresh Dead")
	end
...
function GM:DoPlayerDeath(pl, attacker, dmginfo)
	...
	elseif plteam == TEAM_HUMAN then
		...
		pl.CanPlayerPropkill = CurTime() + 5
...
VoiceSetTranslate["models/player/police.mdl"] = VOICESET_METROPOLICE
VoiceSetTranslate["models/player/police_fem.mdl"] = VOICESET_METROPOLICE
...
VoiceSetTranslate["models/player/kleiner.mdl"] = VOICESET_KLEINER
VoiceSetTranslate["models/mailer/characters/fiddlesticks.mdl"] = VOICESET_FIDDLE
VoiceSetTranslate["models/shadow.mdl"] = VOICESET_TUBE
VoiceSetTranslate["models/vinrax/player/billy_herrington.mdl"] = VOICESET_BILLY
VoiceSetTranslate["models/player/mrbeast/mrbeast.mdl"] = VOICESET_BILLY

VoiceSetTranslate["models/player/arctic.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/guerilla.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/leet.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/phoenix.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/gasmask.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/riot.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/swat.mdl"] = VOICESET_CSS
VoiceSetTranslate["models/player/urban.mdl"] = VOICESET_CSS
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		if self.AntiStallActive and not pl.Revived then
			pl:SetZombieClassName("Super Zombie ZS")
		...
		elseif classtab.UsePlayerModel then
			if table.HasValue(self.RestrictedModels, mdl) or pl:IsBot() then
				pl:SelectRandomPlayerModel()
			end

			local mdl = player_manager.TranslatePlayerModel(pl:GetInfo("cl_playermodel"))
			if not pl.SkinAdjusted and not pl:IsBot() then
				pl:SetModel(mdl)
			end
		elseif classtab.UsePreviousModel then
			local curmodel = string.lower(pl:GetModel())
			if table.HasValue(self.RestrictedModels, curmodel) and not VoiceSetTranslate[curmodel] then
				pl:SelectRandomPlayerModel()
			end
		...
		local wave = self:GetWave() or 1
		local healthPerWave = classtab.HealthPerWave or 0
		local healthPerHuman = classtab.HealthPerHuman or 0
		local numHumanPlayers = team.NumPlayers(TEAM_HUMAN)

		local baseHealth = classtab.Health + ((wave - 1) * healthPerWave) + (healthPerHuman * numHumanPlayers)
    	pl:SetHealth(baseHealth * (pl.m_HealthMulZS or 1))
	...
	elseif pl:Team() == TEAM_HUMAN then
		...
		if pl:IsSkillActive(SKILL_BLOODSTART) then
			pl:SetBloodArmor(pl:GetMaxBloodArmor())
		end
		...
		if self.ZombieEscape then
			...
			elseif pl.m_PreRedeem then
				if self.RedeemLoadout then
					...
					pl:Give("weapon_zs_dualpistol")
					pl:Give("weapon_zs_knife")
				end
			end
		end
	...
	if #UnlockedClasses > 0 then
		for _, pl in pairs(player.GetAll()) do
			net.Start("zs_classunlock")
				net.WriteInt(table.Count(UnlockedClasses), 6)
			net.Send(pl)
		end
	end
end
...
function GM:WaveStateChanged(newstate)
	if newstate then
		...
		if self.ZombieEscape then
			...
			gamemode.Call("SetWaveEnd", self:GetWaveStart() + self:GetWaveOneLength() + (self:GetWave() - 1) * self.TimeAddedPerWave)
		end

		net.Start("zs_wavestart")
			net.WriteUInt(self:GetWave(), 3)
			net.WriteFloat(self:GetWaveEnd())
		net.Broadcast()

		for _, pl in pairs(team.GetPlayers(TEAM_UNDEAD)) do
			pl.m_LastWaveStartSpawn = CurTime()
			if pl:GetZombieClassTable().Name == "Crow" or pl:GetZombieClassTable().Name == "Pigeon" then
	...
	elseif self:GetWave() >= self:GetNumberOfWaves() then -- Last wave is over
		...
		for _, pl in pairs(player.GetAll()) do
			if pl:Team() == TEAM_HUMAN and pl:Alive() then
				if pl:IsSkillActive(SKILL_COMPOSURE) then
					local heal = 35
					local excess = (pl:Health() + heal) - pl:GetMaxHealth()

					local sta = pl:GiveStatus("regeneration")
					sta:AddRegeneration(math.Clamp(excess, 0, heal))

					if pl:Health() < pl:GetMaxHealth() then
						pl:EmitSound("items/smallmedkit1.wav")
						pl:SetHealth(math.min(pl:GetMaxHealth(), pl:Health() + heal))
					end
				end
				if pointsbonus then
			...
			elseif pl:Team() == TEAM_UNDEAD and not pl:Alive() and not pl.Revive then
				...
				if math.random(2) == 1 then
					crowindex = GAMEMODE.ZombieClasses["Pigeon"].Index
				end
...
net.Receive("zs_changeclass", function(len, sender)
	if sender:Team() ~= TEAM_UNDEAD or sender.Revive or GAMEMODE.ZombieEscape then return end
	...
	elseif sender:GetZombieClassTable().Name == classname and not sender.DeathClass then
		...
		if suicide and sender:Alive() and GAMEMODE:GetWaveActive() or GAMEMODE:GetWaveEnd() < 0 and not sender:GetZombieClassTable().Boss and gamemode.Call("CanPlayerSuicide", sender) then
			sender:Kill()
		end
	end
end)
