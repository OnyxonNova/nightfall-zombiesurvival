...
function meta:GetPlayerVampirism()
	return self.Vampirism and self.Vampirism:IsValid() and self.Vampirism:GetVampirism() or 0
end

function meta:GetRegeneration()
	return self.Regeneration and self.Regeneration:IsValid() and self.Regeneration:GetRegeneration() or 0
end
...
function meta:MeleeViewPunch(damage)
    ...
    if self:IsValidLivingZombie() and self:GetZombieClassTable().Boss then
        maxpunch = maxpunch * 0.35
        minpunch = minpunch * 0.35
    end
...
function meta:NearArsenalCrate()
	...
	if self.ArsenalZone and self.ArsenalZone:IsValid() or GAMEMODE.TestingMode then return true end
...
function meta:NearRemantler()
	...
	local remantlerents = {}
	table.Add(remantlerents, ents.FindByClass("prop_remantler"))
	table.Add(remantlerents, GAMEMODE:GetUncorruptedSigils())

	for _, ent in pairs(remantlerents) do
		local nearest = ent:NearestPoint(pos)
		if pos:DistToSqr(nearest) <= 10000 and (WorldVisible(pos, nearest) or self:TraceLine(100).Entity == ent) then -- 80^2
			return true
		end
...
function meta:GetResupplyAmmoType()
	...
	if not self.ResupplyChoice then
		...
		if wep:IsValid() then
			...
			if wep.SecondaryAmmoPurchase then
				ammotype = wep:GetSecondaryAmmoCount() < wep:GetPrimaryAmmoCount() and wep:GetSecondaryAmmoTypeString() or wep:GetPrimaryAmmoTypeString()
			end
		end
	end
...
function meta:GetPoints()
	return GAMEMODE.TestingMode and math.huge or self:GetDTInt(1)
end
...
function meta:GetMaxBloodArmor()
	return self:GetDTInt(DT_PLAYER_INT_MAXBLOODARMOR)
end
...
function meta:AddLegDamageExt(damage, attacker, inflictor, type)
	...
	if type == SLOWTYPE_PULSE then
		...
		if SERVER and inflictor.IsImpulseHammer then
			attacker.AccuPulse = (attacker.AccuPulse or 0) + (self:GetFlatLegDamage() - startleg)

			if attacker.AccuPulse > 70 then
...
function meta:ResetSpeed(noset, health)
	...
	if P_Team(self) == TEAM_UNDEAD then
		local speed = math.max(140, self:GetZombieClassTable().Speed - (GAMEMODE.ObjectiveMap and 15 or 0))
		self:SetSpeed(speed)
		return speed
	end
	...
	if self:IsSkillActive(SKILL_LIGHTHAMMER) and wep:IsValid() and wep.HealStrength then
		speed = speed + 10
	end
...
function meta:NearestPropMaker()
	local pos = self:EyePos()

	local propmakers = ents.FindByClass( "prop_propmaker" )
	local min, propmaker = 99999

	for _, ent in pairs( propmakers ) do
		local nearpoint = ent:NearestPoint( pos )
		local trmatch = self:TraceLine( 100 ).Entity == ent
		local dist = trmatch and 0 or pos:DistToSqr( nearpoint )
		if pos:DistToSqr( nearpoint ) <= 10000 and dist < min then
