...
function GM:DrawTargetID(ent, fade)
	...
	y = y + draw.GetFontHeight("ZSHUDFontTiny")
	...
	if ent:Health() then
		...
		local hptxt = math_ceil(ent:Health()).."/"..ent:GetMaxHealth().." ОЗ"

		draw.SimpleTextBlur(hptxt, "ZSHUDFont", x, y, colTemp, TEXT_ALIGN_CENTER)
		y = y + draw.GetFontHeight("ZSHUDFontSmalle") + 3
		local bloodarmor = ent:GetBloodArmor()

		if mywep.BloodHealing and ent:Team() == TEAM_HUMAN then
			util.ColorCopy(COLOR_SOFTRED, colTemp)
			draw.SimpleTextBlur(bloodarmor.."/"..ent:GetMaxBloodArmor().." Брони", "ZSHUDFontSmallester", x, y, colTemp, TEXT_ALIGN_CENTER)
		...
		if ent:Team() == TEAM_HUMAN then
			...
			local shield = ent:GetStatus("medrifledefboost")
			local strength = ent:GetStatus("strengthdartboost")
			if self.MedicalAura then
				if poison >= 1 then
					util.ColorCopy(COLOR_LIMEGREEN, colTemp)
					draw.SimpleTextBlur("(ЯД - " .. math.floor(poison) ..")", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
					y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
				end
				if bleed >= 1 then
					util.ColorCopy(COLOR_SOFTRED, colTemp)
					draw.SimpleTextBlur("(КРОВОТЕЧЕНИЕ - " .. math.floor(bleed) ..")", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
					y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
				end
			end
			if shield and shield:IsValid() and mywep.DefenceGun then
				local shieldtimer = math.max(shield:GetStartTime() + shield:GetDuration() - CurTime(), 0)
				util.ColorCopy(Color(90, 120, 220), colTemp)
				draw.SimpleTextBlur("(ЗАЩИТА - " .. math.Round(shieldtimer, 1) ..")", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
				y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
			end
			if strength and strength:IsValid() and mywep.StrengthGun then
				local strengthtimer = math.max(strength:GetStartTime() + strength:GetDuration() - CurTime(), 0)
				util.ColorCopy(Color(200, 100, 90), colTemp)
				draw.SimpleTextBlur("(СИЛА - " .. math.Round(strengthtimer, 1) ..")", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
				y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 2
			end
	...
	if ent:Team() == TEAM_UNDEAD then
		...
		if holding:IsValid() then
			draw.SimpleTextBlur(string_format("Переноска [%s]", string_match(holding:GetModel(), ".*/(.+)%.mdl") or "объекта"), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
		...
		if remortlevel >= 1 then
			draw.SimpleTextBlur(string_format("Ур. %d П.Ур. %d", level, remortlevel), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
			draw.SimpleTextBlur("Ур. "..level, "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		end
	end
end
...
function GM:DrawSigilTargetHint(ent, fade)
	...
	draw.SimpleTextBlur("Маяк", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
	y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 0

	if self:NumSigils() > 1 then
		draw.SimpleTextBlur("Нажмите E, чтобы телепортироваться", "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
	end
end
