...
function meta:ProcessDamage(dmginfo)
	...
	if P_Team(self) == TEAM_UNDEAD then
		...
		if attacker:IsValidLivingHuman() and inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
			...
			if wep.IsMelee then
				...
				if attacker:IsSkillActive(SKILL_HEAVYSTRIKES) then
					attacker:TakeSpecialDamage(damage * 0.05, DMG_SLASH, self, self:GetActiveWeapon())
				end

				if attacker:IsSkillActive(SKILL_VAMPIRISM) then
					local sta = attacker:GiveStatus("vampirism") 
					sta:AddVampirism(math.ceil(math.min(25 , damage * 0.115))) 
				end

				if not attacker:GetStatus("laststand") and attacker:IsSkillActive(SKILL_LASTSTAND) then
					dmginfo:SetDamage(dmginfo:GetDamage() * 0.8)
				end

				if wep.Culinary and attacker:IsSkillActive(SKILL_MASTERCHEF) and math.random(6) == 1 then
	...
	if bit.band(dmgtype, DMG_ENERGYBEAM) ~= 0 then
		dmginfo:SetDamage(P_Team(self) == TEAM_UNDEAD and dmginfo:GetDamage() or 0)
	end
	...
	if inflictor:IsValid() and (inflictor:IsPhysicsModel() or inflictor.IsPhysbox) and self:IsValidLivingHuman() then
		local damage = dmginfo:GetDamage()
		local forcedamp = self:HasTrinket("forcedamp_q2") or self:HasTrinket("forcedamp_q3")
		...
		if forcedamp or (attacker:IsValidLivingZombie() and not noadj) then
			damage = math.max(16, (math.log10(damage) * 31)-5)
		end

		if self.PhysicsDamageTakenMul then
			damage = damage * self.PhysicsDamageTakenMul * (((attacker.CanPlayerPropkill and CurTime() > attacker.CanPlayerPropkill) and attacker:IsValidZombie()) and 1 or 0)
		end
		...
		if inflictor == attacker:GetActiveWeapon() then
			...
			if bit.band(dmgtype, DMG_SLASH) ~= 0 or inflictor.IsMelee then
				if self.BarbedArmor and self.BarbedArmor > 0 then
					...
					attacker:AddArmDamage(self.BarbedArmor - 5)
				end
				...
				-- this code is ass
				if (self:HasTrinket("reactiveflasher") or self:HasTrinket("reactiveflasher_q1") or self:HasTrinket("reactiveflasher_q2") or self:HasTrinket("reactiveflasher_q3")) and (not self.LastReactiveFlash or self.LastReactiveFlash + self.ReactiveFlasherTimer < CurTime()) then
				...
				elseif (self:HasTrinket("bleaksoul") or self:HasTrinket("bleaksoul_q1") or self:HasTrinket("bleaksoul_q2") or self:HasTrinket("bleaksoul_q3")) and (not self.LastBleakSoul or self.LastBleakSoul + self.BleakSoulTimer < CurTime()) then
	...
	if self:GetBloodArmor() > 0 then
		local damage = dmginfo:GetDamage() * (self:GetStatus("mбуedrifledefboost") and 0.7 or 1)
		if damage > 0 then
			local ratio = 0.5 + self.BloodArmorDamageReductionAdd

			local absorb = math.min(self:GetBloodArmor(), damage * ratio)

			local absorbblood = math.min(self:GetBloodArmor(), ratio > 1 and damage / ratio or damage * ratio)

			dmginfo:SetDamage(damage - absorb)
			self:SetBloodArmor(self:GetBloodArmor() - absorbblood)
	...
	if dmginfo:GetDamage() > 0 and not self:HasGodMode() then
		self.NextRegenTrinket = CurTime() + 16
...
GM.TrinketRecharges = {
	reactiveflasher = {"ReactiveFlashMessage", "LastReactiveFlash", "Реактивная Вспышка", 80},
	reactiveflasher_q1 = {"ReactiveFlashMessage", "LastReactiveFlash", "Реактивная Вспышка", 72},
	reactiveflasher_q2 = {"ReactiveFlashMessage", "LastReactiveFlash", "Реактивная Вспышка", 64},
	reactiveflasher_q3 = {"ReactiveFlashMessage", "LastReactiveFlash", "Реактивная Вспышка", 56},
	biocleanser = {"BioCleanserMessage", "LastBioCleanser", "Биоочистка", 24},
	biocleanser_q1 = {"BioCleanserMessage", "LastBioCleanser", "Биоочистка", 21},
	biocleanser_q2 = {"BioCleanserMessage", "LastBioCleanser", "Биоочистка", 18},
	biocleanser_q3 = {"BioCleanserMessage", "LastBioCleanser", "Биоочистка", 15},
	bleaksoul = {"BleakSoulMessage", "LastBleakSoul", "Мрачная Душа", 40},
	bleaksoul_q1 = {"BleakSoulMessage", "LastBleakSoul", "Мрачная Душа", 35},
	bleaksoul_q2 = {"BleakSoulMessage", "LastBleakSoul", "Мрачная Душа", 30},
	bleaksoul_q3 = {"BleakSoulMessage", "LastBleakSoul", "Мрачная Душа", 25}
}
...
function meta:CheckTrinketRecharges()
	...
	for trinket, data in pairs(GAMEMODE.TrinketRecharges) do
		...
		if self:HasTrinket(trinket) and not self[msg] and self[last] and time > self[last] + delay then
			self:CenterNotify({killicon = "weapon_zs_trinket"}, " ", COLOR_ORANGE, translate.ClientFormat(self, "trinket_recharged", name))
...
function meta:GetBossZombieIndex()
	...
	if desired == "[RANDOM]" or desired == "" then
		desired = "Nightmare"
	end
...
function meta:SetMaxBloodArmor(armormax)
	self:SetDTInt(DT_PLAYER_INT_MAXBLOODARMOR, armormax)
end
...
function meta:ChangeToCrow()
	...
	if math.random(2) == 1 then
		crowclass = GAMEMODE.ZombieClasses["Pigeon"]
	end
...
function meta:GiveStatus(sType, fDie)
	...
	if resistable and (self:HasTrinket("biocleanser") or self:HasTrinket("biocleanser_q1") or self:HasTrinket("biocleanser_q2") or self:HasTrinket("biocleanser_q3")) and (not self.LastBioCleanser or self.LastBioCleanser + self.BioCleanserTimer < CurTime()) then
	...
	local hemostasisfDie = self:GetBloodArmor() >= 2 and resistable and self:IsSkillActive(SKILL_HAEMOSTASIS) and fDie * 0.5

	local cur = self:GetStatus(sType)
	if cur then
		if fDie then
			cur:SetDie(hemostasisfDie or fDie)
		end
		...
		if ent:IsValid() then
			ent:Spawn()
			if fDie then
				ent:SetDie(hemostasisfDie or fDie)
			end
			ent:SetPlayer(self)
			return ent
		end
...
function meta:DropAll()
	if GAMEMODE.TestingMode then return end
...
function meta:Resupply(owner, obj)
	...
	net.Start("zs_stowagecaches")
		net.WriteUInt(self.StowageCaches, 6)
		net.WriteBool(false)
	net.Send(self)
	...
	if self:IsSkillActive(SKILL_NAILSAMMUNITION) and math.random(4) == 4 then
		self:GiveAmmo(3, "GaussEnergy")
	end
	...
	if self ~= owner and owner:IsValidHuman() then
		...
		owner:AddPoints(0.5, nil, nil, true)
...
function meta:MakeBossDrop()
	local drop = table.Random(GAMEMODE.EssenceTrinkets)
...
function meta:PulseResonance(attacker, inflictor)
	...
	timer.Create("PulseResonance" .. attacker:UniqueID(), 0.06, 1, function()
		...
		if attacker:IsValidLivingHuman() then
			util.BlastDamagePlayer(inflictor, attacker, pos, 100, 75, DMG_ENERGYBEAM, 0.95)
...
function meta:CryogenicInduction(attacker, inflictor, damage)
	...
	timer.Create("Cryogenic" .. attacker:UniqueID(), 0.06, 1, function()
		...
		if attacker:IsValidLivingHuman() then
			util.BlastDamagePlayer(inflictor, attacker, pos, 100, self:GetMaxHealthEx() * 0.12, DMG_ENERGYBEAM, 0.95)
			for _, ent in pairs(util.BlastAlloc(inflictor, attacker, pos, 100 * (attacker.ExpDamageRadiusMul or 1))) do
				if ent:IsValidLivingPlayer() and gamemode.Call("PlayerShouldTakeDamage", ent, attacker) then
