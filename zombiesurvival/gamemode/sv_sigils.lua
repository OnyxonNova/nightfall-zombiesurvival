...
function GM:OnSigilCorrupted(ent, dmginfo)
	net.Start("zs_sigilcorrupted")
		net.WriteUInt(self:NumCorruptedSigils(), 2)
	net.Broadcast()
end
...
function GM:CreateSigils(secondtry, rearrange)
	...
	if self.ZombieEscape or self.ObjectiveMap then
		self:SetUseSigils(false)
		return
	end

	self:SigilCountControl()

	if alreadycreated >= self.GetMaxSigils() and not rearrange then return end
	...
	for i = 1 + (rearrange and 0 or alreadycreated), self.GetMaxSigils() do
		local id
