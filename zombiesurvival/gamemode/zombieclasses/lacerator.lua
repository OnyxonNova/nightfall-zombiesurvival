...
CLASS.GearFoleyVolumeHeavy = 0.4
CLASS.GearFoleyVolume = 0.6
...
function CLASS:PlayerFootstep(pl, vFootPos, iFoot, strSoundName, fVolume, pFilter)
	if iFoot == 0 then
		pl:EmitSound("npc/antlion_guard/foot_heavy1.wav", 70, math_random(120,133), self.GearFoleyVolumeHeavy)
	else
		pl:EmitSound("npc/antlion_guard/foot_heavy2.wav", 70, math_random(120,133), self.GearFoleyVolumeHeavy)
	end
	pl:EmitSound(self.GearFoley[math_random(#self.GearFoley)], 70, 100, self.GearFoleyVolume)
...
function CLASS:UpdateAnimation(pl, velocity, maxseqgroundspeed)
	...
	if wep.IsRoaring and wep:IsRoaring() and velocity:Length2DSqr() <= 1 then
		pl:SetPlaybackRate(0)
