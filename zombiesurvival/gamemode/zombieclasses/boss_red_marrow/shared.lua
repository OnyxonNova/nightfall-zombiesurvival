...
CLASS.Health = 2350
CLASS.HealthPerWave = 225
CLASS.HealthPerHuman = 175
CLASS.Speed = 170
...
function CLASS:ProcessDamage(pl, dmginfo)
    ...
    local numthreshold = math_Clamp(math_ceil(hp / 380), 1, 5)
	local dmgthreshold = math_Clamp(numthreshold * 380 - 380, 1, 1900)
    ...
    if newhp <= dmgthreshold and pl["bloodth"..numthreshold] then
        ...
        pl:GiveStatus("shieldrmarrow", 7)
        pl:GiveStatus("zombie_battlecry", 7)
        ...
        pl:GodEnable()
        util.BlastDamageEx(pl, pl, pl:GetPos(), 128, 22, DMG_CLUB)
        pl:GodDisable()
    end
end
