...
function PANEL:Init()
	...
	self.m_ServerNameLabel.Font = "ZSScoreBoardTitle"
	...
	self.m_MapRoundTime = vgui.Create("DLabel", self)
    self.m_MapRoundTime.Font = "ZSScoreBoardSubTitle"
    self.m_MapRoundTime:SetFont(self.m_MapRoundTime.Font)
    self.m_MapRoundTime:SetText(game.GetMap() .. " - " .. util.ToMinutesSecondsCD(CurTime()))
    self.m_MapRoundTime:SetTextColor(COLOR_GRAY)
    self.m_MapRoundTime:NoClipping(true)
    self.m_MapRoundTime.Paint = BlurPaint

	self.m_AuthorLabel = EasyLabel(self, 'by William "JetBoom" Moodhe (williammoodhe@gmail.com)', "ZSScoreBoardPing", COLOR_GRAY)

	self.m_HumanHeading = vgui.Create("DTeamHeading", self)
	self.m_HumanHeading:SetTeam(TEAM_SURVIVORS)

	self.m_ZombieHeading = vgui.Create("DTeamHeading", self)
	self.m_ZombieHeading:SetTeam(TEAM_ZOMBIES)

	self.m_FragsLabel = EasyLabel(self, translate.Get("score_label"), "ZSScoreBoardPlayer", COLOR_GRAY)

	self.m_RemortCLabel = EasyLabel(self, "RLvl", "ZSScoreBoardPlayer", COLOR_GRAY)

	self.m_BrainsLabel = EasyLabel(self, translate.Get("brains_label"), "ZSScoreBoardPlayer", COLOR_GRAY)
	self.m_RemortCZLabel = EasyLabel(self, "RLvl", "ZSScoreBoardPlayer", COLOR_GRAY)

	self.ZombieList = vgui.Create("DScrollPanel", self)
	self.ZombieList.Team = TEAM_ZOMBIES

	self.HumanList = vgui.Create("DScrollPanel", self)
	self.HumanList.Team = TEAM_SURVIVORS
end
...
function PANEL:PerformLayout()
	...
	self.m_MapRoundTime:SizeToContents()
    self.m_MapRoundTime:SetPos(math.min(self:GetWide() - self.m_MapRoundTime:GetWide(), self:GetWide() * 0.75 - self.m_MapRoundTime:GetWide() * 0.5), 48 - self.m_MapRoundTime:GetTall() / 2)

    self.m_ServerNameLabel:SizeToContents()
    self.m_ServerNameLabel:SetPos(math.min(self:GetWide() - self.m_ServerNameLabel:GetWide(), self:GetWide() * 0.75 - self.m_ServerNameLabel:GetWide() * 0.5), 20 - self.m_ServerNameLabel:GetTall() / 2)
	...
	self.m_FragsLabel:SizeToContents()
	self.m_FragsLabel:SetPos((self:GetWide() / 2 - 24) * 0.563 - self.m_FragsLabel:GetWide() * 0.5, 110 * screenscale - self.m_HumanHeading:GetTall())
	self.m_FragsLabel:MoveBelow(self.m_HumanHeading, 1 * screenscale)

	self.m_RemortCLabel:SizeToContents()
	self.m_RemortCLabel:SetPos((self:GetWide() / 2 - 24) * 0.765 - self.m_RemortCLabel:GetWide() * 0.5, 110 * screenscale - self.m_HumanHeading:GetTall())
	self.m_RemortCLabel:MoveBelow(self.m_HumanHeading, 1 * screenscale)

	self.m_RemortCZLabel:SizeToContents()
	self.m_RemortCZLabel:SetPos(self:GetWide() / 2 + 25 * screenscale + (self:GetWide() / 2 - 24) * 0.73 - self.m_RemortCZLabel:GetWide() * 0.5, 110 * screenscale - self.m_HumanHeading:GetTall())
	self.m_RemortCZLabel:MoveBelow(self.m_ZombieHeading, 1 * screenscale)

	self.m_BrainsLabel:SizeToContents()
	self.m_BrainsLabel:SetPos(self:GetWide() / 2 + 3 * screenscale + (self:GetWide() / 2 - 24) * 0.558 - self.m_BrainsLabel:GetWide() * 0.4, 110 * screenscale - self.m_HumanHeading:GetTall())
...
function PANEL:CreatePlayerPanel(pl)
	...
	local panel = vgui.Create("ZSPlayerPanel", pl:Team() == TEAM_ZOMBIES and self.ZombieList or self.HumanList)
	panel:SetPlayer(pl)
	panel:Dock(TOP)
...
function PANEL:Init()
	...
	self.m_AvatarButton:SetSize(36 * screenscale, 36 * screenscale)
	...
	self.m_ClassImage:SetTooltipPanelOverride("DExTooltip")
...
function PANEL:PerformLayout()
	self.m_AvatarButton:AlignLeft(19)
	...
	if pl:IsValid() and not pl:IsBot() then
        ...
        self.m_PlayerLabel:MoveRightOf(self.m_SpecialImage, 4)
    end
	...
	self.m_SpecialImage:SetPos(self:GetWide() * 0.0225 - self.m_SpecialImage:GetWide() / 2, 0)
	self.m_SpecialImage:CenterVertical()

	self.m_ClassImage:SetSize(self:GetTall(), self:GetTall())
	self.m_ClassImage:SetPos(self:GetWide() * 0.7 - self.m_ClassImage:GetWide() * 1, 0)
	self.m_ClassImage:CenterVertical()

	local pingsize = self:GetTall() - 10

	self.m_PingMeter:SetSize(pingsize, pingsize)
	self.m_PingMeter:AlignRight(6)
	...
	self.m_RemortLabel:SetPos(self:GetWide() * 0.775 - self.m_RemortLabel:GetWide() * 1, 0)
...
function PANEL:RefreshPlayer()
	...
	self.m_ScoreLabel:SetText((pl:Team() == TEAM_SURVIVORS) and (pl:GetPoints().." / ".. pl:Frags()) or pl:Frags())
	...
	if MySelf:Team() == TEAM_ZOMBIES and pl:Team() == TEAM_ZOMBIES and pl:GetZombieClassTable().Icon then
	...
	if pl:Team() ~= self._LastTeam then
		...
		self:SetParent(self._LastTeam == TEAM_SURVIVORS and ScoreBoard.HumanList or ScoreBoard.ZombieList)
	end
