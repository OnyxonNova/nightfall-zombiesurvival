...
GM.Credits = {
	{"Onyxon", "Discord: onyxon_nova", "Code Contributor", "76561198963426663"},
	{"Various NZS Contributors", "", "Various contributions"},
	
	{"", "", "", "", true},
	
	{"William \"JetBoom\" Moodhe", "williammoodhe@gmail.com (www.noxiousnet.com)", "Creator / Programmer", "76561197966880749"},
...
team.SetUp(TEAM_SURVIVORS, "Выжившие", Color(0, 160, 255, 255))
...
GM.SoundDuration = {
	["zombiesurvival/music_win.ogg"] = 45.749,
	["zombiesurvival/music_lose_1.ogg"] = 50.714,
	["zombiesurvival/lasthuman.ogg"] = 124.503,
...
function GM:AddCustomAmmo()
	...
	game.AddAmmoType({name = "propmaker"})
	...
	game.AddAmmoType({name = "foodbeer"})
end
...
function GM:DynamicSpawnIsValid(ent, humans, allplayers)
	...
	local is_nest = ent:GetClass() == "prop_creepernest" or ent:GetClass() == "prop_bigcreepernest"
...
function GM:GetDynamicSpawns(pl)
	...
	for _, big_nest in pairs(ents.FindByClass("prop_bigcreepernest")) do
        if self:DynamicSpawnIsValid(big_nest, humans) then
            table.insert(tab, big_nest)
        end
    end
...
function GM:OnPlayerHitGround(pl, inwater, hitfloater, speed)
	...
	if isundead then
		...
		threshold_mul = (pl.FallDamageThresholdMul or 1) - (anchormul)
		slowdown_mul = pl.FallDamageSlowDownMul or 1
		recovery_mul = pl.FallDamageRecoveryMul or 1
		damage_mul = (pl.FallDamageDamageMul or 1) + (anchormul)
	end
	...
	if SERVER then
		...
		if groundent:IsValid() and groundent:IsPlayer() and PTeam(groundent) == TEAM_UNDEAD and pl:HasTrinket("weights") then
...
function GM:PlayerCanBeHealed(pl)
	return pl:Health() < pl:GetMaxHealth() or pl:GetPoisonDamage() > 0 or pl:GetBleedDamage() > 0
end

function GM:PlayerCanBeHealedBlood(pl)
	return pl:GetBloodArmor() < pl:GetMaxBloodArmor()
end
...
function GM:GetDamageResistance(fearpower)
	if self:NumSigils() > 0 and self:GetUseSigils() then
		return fearpower * 0.1 + self:NumSigilsCorrupted() / self:NumSigils() * 0.2
	end
...
local SpecialTagTab = {
	["superadmin"] = {"VGUI/servers/icon_robotron", "Админ"}
}

function GM:IsSpecialPerson(pl, image)
	local img = "icon16/user.png"
	local tooltip = "Игрок"

	local group = pl:GetUserGroup()

	if SpecialTagTab[group] then
		img = SpecialTagTab[group][1]
		tooltip = SpecialTagTab[group][2]
	elseif pl:IsBot() then
		img = "icon16/transmit.png"
		tooltip = "Бот"
	end
