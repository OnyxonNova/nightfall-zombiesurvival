...
function meta:HealPlayer(pl, amount, pointmul, nobymsg, poisononly)
	...
	local healrec = (pl.HealingReceived or 1) - (pl:GetStatus("sickness") and 0.5 or 0)
...
function meta:HealBloodPlayer(pl, amount, pointmul, nobymsg)
	local healed, rmv = 0, 0

	local health, maxhealth = pl:GetBloodArmor(), pl:GetMaxBloodArmor()
	local missing_health = maxhealth - health
	...
	if healed > 0 and self:IsPlayer() then
		gamemode.Call("PlayerHealedBloodTeamMember", self, pl, healed, self:GetActiveWeapon(), pointmul, nobymsg, healed >= 10)
	end
...
function meta:PackUp(pl)
	...
	if status:IsValid() then
		...
		if self.GetObjectOwner then
			...
			if owner:IsValid() and owner:Team() == TEAM_HUMAN and owner ~= pl and not pl:IsAdmin() then
